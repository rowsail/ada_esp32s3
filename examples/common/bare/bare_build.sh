...
# EXT_RAM_BSS_SIZE=<bytes> : reserve a slice at the BOTTOM of the mapped PSRAM
# window for the .ext_ram.bss output section, and start the PSRAM heap above it.
# Nothing places that section otherwise, so an object with
# `Linker_Section => ".ext_ram.bss"` silently lands at a bogus address -- which is
# how libs/tls's TLS_BUFFERS=psram variant crashes (LoadProhibited, vaddr near 0).
# The generated fragment goes in via the same $EXTRA_LD path the psram example
# uses.  Not zeroed: Ada elaboration writes whatever needs a defined value, and
# the buffers placed here are written before they are read.
EXT_RAM_BSS_LD=""
if [ "${EXT_RAM_BSS_SIZE:-0}" -gt 0 ] 2>/dev/null; then
    [ "${BOARD_PSRAM_SIZE:-0}" -gt 0 ] \
        || { echo "[bare] EXT_RAM_BSS_SIZE needs PSRAM_Size>0 in board.ads" >&2; exit 1; }
    [ "$EXT_RAM_BSS_SIZE" -lt "$BOARD_PSRAM_SIZE" ] \
        || { echo "[bare] EXT_RAM_BSS_SIZE >= PSRAM size" >&2; exit 1; }
fi
...
if [ -n "$HEAP_SIZE" ]; then
    ...
    if [ "${HEAP_PSRAM:-0}" != 0 ]; then
        ...
        # The window is carved bottom-up: [.ext_ram.bss][heap][env stack].
        PHEAP_BASE=$(( 0x3D000000 + ${EXT_RAM_BSS_SIZE:-0} ))
        PHEAP_SIZE=$(( PSRAM_HEAP_SIZE - ${EXT_RAM_BSS_SIZE:-0} ))
        PEND=$(( PHEAP_BASE + PHEAP_SIZE ))
        BHEAP_DEFSYM="-Wl,--defsym=__bare_heap_base=$PHEAP_BASE -Wl,--defsym=__bare_heap_end=$PEND"
        printf '[bare]      heap -> PSRAM (%s B @ 0x%x)\n' "$PHEAP_SIZE" "$PHEAP_BASE"
    else
...
if [ "${EXT_RAM_BSS_SIZE:-0}" -gt 0 ] 2>/dev/null; then
    EXT_RAM_BSS_LD="$OBJ/ext_ram_bss.ld"
    cat > "$EXT_RAM_BSS_LD" <<EOF
/* Generated by bare_build.sh from EXT_RAM_BSS_SIZE=$EXT_RAM_BSS_SIZE.  A slice at
 * the bottom of the bootloader-mapped PSRAM window; the heap starts above it
 * (see __bare_heap_base).  Same d-bus window and rationale as
 * examples/esp32s3_psram/psram.ld. */
MEMORY
{
  ext_ram_bss_seg (RW) : org = 0x3D000000, len = $EXT_RAM_BSS_SIZE
}
SECTIONS
{
  .ext_ram.bss (NOLOAD) :
...
EOF
    EXTRA_LD_ARGS="$EXTRA_LD_ARGS -T $EXT_RAM_BSS_LD"
    echo "[bare]      .ext_ram.bss -> PSRAM ($EXT_RAM_BSS_SIZE B @ 0x3D000000)"
fi
