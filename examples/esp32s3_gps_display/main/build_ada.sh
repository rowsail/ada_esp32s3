#!/bin/bash
# Build the GPS-display-demo Ada (gps_display.gpr) against the PINNED crate runtime into
...
EX="$(cd "$HERE/.." && pwd)"                 # examples/esp32s3_gps_display/
...
( cd "$EX" && gprbuild -p -P gps_display.gpr )
