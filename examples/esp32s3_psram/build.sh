...
# src/psram_board.adb (bare_board_init) only re-applies the cache-MMU map (after
# start.S wipes it) for big.adb's 1 MB .ext_ram.bss array, placed at 0x3D000000 by
# psram.ld.
# No HEAP_SIZE: this is light-tasking (no heap), and its runtime already provides
# memcpy.  NEED_BARE_MEM links the shared Ada Bare_Mem for the memset libgcc pulls.
HERE="$(cd "$(dirname "$0")" && pwd)"
export ENV_STACK_SIZE=98304            # generous env stack (kept; the app itself is light)
export NEED_BARE_MEM=1                  # freestanding memset (was in glue.c) via Bare_Mem
