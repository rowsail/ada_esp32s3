...
package body Big is
   ...
   procedure Report (Addr, Bytes, Checksum : Unsigned_32) is
      ...
      Where : constant String :=
        (if Top = 16#3C# or else Top = 16#3D#
         then "PSRAM"
         elsif Top = 16#3F#
         then "internal-SRAM"
         else "?");
   begin
      Put ("[psram] buffer @ 0x");
      Put_Hex (Addr, Width => 8);
      Put ("  ");
      Put_Unsigned (Bytes);
      Put (" bytes  checksum=0x");
      Put_Hex (Checksum, Width => 8);
      Put ("  (");
      Put (Where);
      Put_Line (")");
   end Report;
   ...
   begin
      ...
      Report (Unsigned_32 (To_Integer (Buffer'Address)), Buffer'Length, Sum);
   end Run;
