...
begin
   ...
   declare
      ...
      Bars : constant array (0 .. Bar_Count - 1) of Display_Driver.Color :=
        (Display_Driver.Red,
         Display_Driver.Green,
         Display_Driver.Blue,
         Display_Driver.White,
         Yellow,
         Cyan,
         Magenta,
         Display_Driver.Black);
   begin
      for I in Bars'Range loop
         Display_Driver.Fill_Rect
           (Display_Session,
            X => I * Bar_Width,
            Y => 0,
            W => Bar_Width,
   ...
   begin
      ...
      Display_Driver.Text.Draw_Text
        (Display_Session,
         X     => 6,
         Y     => 120,
         Str   => "40 MHz SPI" & Line_Feed & "240x240" & Line_Feed & "write-only",
         FG    => Caption_Amber,
         BG    => Text_Background,
         Scale => 2);
   end;
   ...
   --  Clipping self-test (the last, persistent screen): a 60x60 bordered bitmap
   --  (2 px white frame on red) drawn so it OVERHANGS the panel edges.  Correct
   --  clipping draws only the visible sub-rectangle -- so the reference copy at
   --  top-left is a full bordered box, and the two overhanging copies show clean
   --  partial boxes with straight edges.  Before the clipping fix, an oversized
   ...
   Put_Line ("[lcd] done -- check the panel (clip test is the final screen).");
