...
procedure Main is
   ...
   Framebuffer : LCD.Color_Array (0 .. FB_Width * FB_Width - 1) := (others => LCD.Black);
   ...
   function Sin (Angle : Integer) return Integer
   is (Sine_Table (Angle mod Table_Steps));
   function Cos (Angle : Integer) return Integer
   is (Sine_Table ((Angle + Quarter_Turn) mod Table_Steps));
   ...
   function Rotate (V : Vector_3D; Cos_X, Sin_X : Integer; Cos_Y, Sin_Y : Integer) return Vector_3D
   is
         ...
         begin
            for K in 0 .. 3 loop
               ...
               begin
                  ...
                  if (Ay <= Y and then Y < By) or else (By <= Y and then Y < Ay) then
                     declare
                        --  X where this edge crosses scanline Y.
                        X : constant Integer := Ax + (Bx - Ax) * (Y - Ay) / (By - Ay);
                     begin
   ...
   begin
      ...
      begin
         for I in Faces'Range loop
            Visible (I) :=
              Rotate (Faces (I).Normal, Cos_X, Sin_X, Cos_Y, Sin_Y).Z * Eye > One * One;
         end loop;
         ...
         for I in Faces'Range loop
            if Visible (I) then
               for K in 0 .. 3 loop
                  ...
                  begin
                     Line (Screen_X (A), Screen_Y (A), Screen_X (B), Screen_Y (B), LCD.Black);
                  end;
               end loop;
            end if;
         end loop;
   ...
   begin
      if Window_X > Prev_Window_X then
         LCD.Fill_Rect
           (Session, Prev_Window_X, Prev_Window_Y, Window_X - Prev_Window_X, FB_Width, LCD.Black);
      ...
      if Window_Y > Prev_Window_Y then
         LCD.Fill_Rect
           (Session, Prev_Window_X, Prev_Window_Y, FB_Width, Window_Y - Prev_Window_Y, LCD.Black);
...
begin
   ...
   LCD.Setup (Device, Sclk => Sclk_Pin, Mosi => Mosi_Pin, DC => Data_Cmd_Pin, CS => Chip_Sel_Pin);
   ...
   loop
      ...
      Render_Cube;
      Clear_Trail;
      LCD.Draw_Bitmap (Session, Window_X, Window_Y, FB_Width, FB_Width, Framebuffer);
