...
package body ESP32S3.W5500.Net_Device is
   ...
   function N (St : WS.Status) return Net_Devices.Status
   is (Net_Devices.Status'Val (WS.Status'Pos (St)));

   function To_W (A : Net_Devices.IPv4_Address) return IPv4_Address
   is (IPv4 (Byte (A (0)), Byte (A (1)), Byte (A (2)), Byte (A (3))));

   function To_N (A : IPv4_Address) return Net_Devices.IPv4_Address
   is (0 => Net_Devices.Octet (A (0)),
       1 => Net_Devices.Octet (A (1)),
       2 => Net_Devices.Octet (A (2)),
       3 => Net_Devices.Octet (A (3)));
   ...
   overriding
   function Socket_Count (Self : Instance) return Positive is
   ...
   overriding
   function Local_IP (Self : Instance) return Net_Devices.IPv4_Address
   is (To_N (Get_IP (Self.Dev.all)));
   ...
   overriding
   function Is_Up (Self : Instance) return Boolean is
   begin
      return
        Self.Dev /= null
   ...
   overriding
   procedure Close (Self : in out Instance; Index : Natural) is
   begin
