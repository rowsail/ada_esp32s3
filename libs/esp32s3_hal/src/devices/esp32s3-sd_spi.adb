...
package body ESP32S3.SD_SPI is
   ...
   --  A command/response frame has bytes in it that carry no information we
   --  act on, and the SPI transfer that clocks them still has to happen: a
   --  response must be consumed in full before the next command, a token has to
   --  go out, and one trailing byte after CS rises releases the card's MISO.
   --  These three name that traffic, so a discarded byte reads as deliberate
   --  rather than as a result somebody forgot to check.

   --  Clock Count bytes past without inspecting them.
   procedure Skip (C : Card; S : ESP32S3.SPI.Session; Count : Positive := 1) is
   begin
      for Each in 1 .. Count loop
         declare
            Ignored : constant Unsigned_8 := Swap (C, S, 16#FF#);
   ...
   --  Put one byte on the bus (a gap byte, a start-of-block token).  What comes
   --  back on MISO during it is not a response.
   procedure Put (C : Card; S : ESP32S3.SPI.Session; B : Unsigned_8) is
      Ignored : constant Unsigned_8 := Swap (C, S, B);
      pragma Unreferenced (Ignored);
   begin
      null;
   end Put;
   ...
   --  Issue a command whose R1 we do not act on: CMD55 only announces that an
   --  ACMD follows, and CMD16's effect shows up on the next read.
   procedure Issue
     (C : Card; S : ESP32S3.SPI.Session; Cmd : Unsigned_8; Arg : Unsigned_32)
   is
      Ignored : constant Unsigned_8 := Command (C, S, Cmd, Arg);
      pragma Unreferenced (Ignored);
   begin
      null;
   end Issue;
   ...
   begin
      ...
      if (Response and R1_Illegal) /= 0 then
         ...
         begin
            --  R7 is five bytes: the R1 above, then two reserved, then the
            --  voltage byte, then the check pattern.  Only the last is ours.
            Skip (C, Session, 3);
      ...
      if not C.Block_Addressed then
         Issue (C, Session, CMD16, 512);
      end if;

      ESP32S3.GPIO.Set (C.Cs);
      Skip (C, Session);                       --  trailing clocks to release MISO
      ESP32S3.SPI.Release (Session);
   ...
   begin
      ...
      Skip (C, Session, 2);                    --  the block's 2 CRC bytes

      ESP32S3.GPIO.Set (C.Cs);
      Skip (C, Session);                       --  trailing clocks
      ESP32S3.SPI.Release (Session);
      Result := OK;
   end Read_Block;
   ...
   begin
      ...
      Put (C, Session, 16#FF#);                    --  one gap byte before the token
      Put (C, Session, Data_Token);                --  start-of-block token

      Tx_Buf (C.Host) := Buf (Data);           --  512 data bytes
      Shift (C, Session, 512);

      Skip (C, Session, 2);                        --  2 dummy CRC bytes
