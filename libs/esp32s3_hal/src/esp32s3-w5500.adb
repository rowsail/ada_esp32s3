...
package body ESP32S3.W5500 is
   ...
   use type ESP32S3
              .GPIO
              .Pad_Number;   --  make "/=" visible for the No_Pin checks
   ...
   Scratch_Tx : array (ESP32S3.SPI.SPI_Host) of Frame :=
     (others => (others => 0));
   Scratch_Rx : array (ESP32S3.SPI.SPI_Host) of Frame :=
     (others => (others => 0));
   ...
   begin
      ...
      while Off <= Data'Last loop
         declare
            N : constant Natural :=
              Natural'Min (Chunk_Size, Data'Last - Off + 1);
         begin
   ...
   procedure Write_U8
     (Dev : Device; Blk : Block; Addr : Interfaces.Unsigned_16; V : Byte) is
   begin
   ...
   begin
      ...
      if Rst /= ESP32S3.GPIO.No_Pin then
         ESP32S3.GPIO.Configure
           (ESP32S3.GPIO.Pin_Id (Rst),
            Mode => ESP32S3.GPIO.Output,
            Pull => ESP32S3.GPIO.Pull_Up);
         ESP32S3.GPIO.Set
           (ESP32S3.GPIO.Pin_Id (Rst));     --  RSTn high = not reset
   ...
   begin
      if Dev.Rst /= ESP32S3.GPIO.No_Pin then
         ESP32S3.GPIO.Clear
           (ESP32S3.GPIO.Pin_Id (Dev.Rst));  --  RSTn low = reset
         delay until
           Clock + Microseconds (600);              --  hold >= 500 us
         ESP32S3.GPIO.Set (ESP32S3.GPIO.Pin_Id (Dev.Rst));
         delay until
           Clock + Milliseconds (2);                --  PLL lock / settle

      else
         Write_U8
           (Dev, Common_Regs, MR, MR_RST);             --  software reset
