...
package body ESP32S3.GDMA is
   ...
   --  Only the registers this driver programs are declared; the rest of the
   --  192-byte channel window (0x78, 0x7C, 0x84 .. 0xA4) is left as holes on
   --  purpose rather than padded with dummy components, so the record reads as
   --  the register map and not as a struct.  'Size below pins the stride, which
   --  is what Channel_Array indexing depends on.
   pragma Warnings (Off, "* bits of ""Channel_Regs"" unused");
   for Channel_Regs'Size use 16#C0# * 8;          --  192-byte channel stride
   pragma Warnings (On, "* bits of ""Channel_Regs"" unused");
