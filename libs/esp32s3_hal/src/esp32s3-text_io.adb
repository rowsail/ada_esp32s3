...
package body ESP32S3.Text_IO is
   ...
   function Digit_Val (C : Character) return Integer
   is (case C is
            ...
            begin
               for I in S'Range loop
                  ...
                  if N = Chunk'Length then
                     B.FS.Append (B.Node, Chunk (0 .. N - 1));
                     N := 0;
                  end if;
               end loop;
               if N > 0 then
   ...
   begin
      ...
      Skip_Blanks (B);
      Peek_Wait (B, C, Av);
      if Av and then (C = '-' or else C = '+') then
         Neg := C = '-';
         Advance (B);
      end if;
      loop
   ...
   begin
      ...
      while I <= F'Last loop
         declare
            J : Natural := I;
         begin
            while J <= F'Last and then F (J) /= ',' loop
               J := J + 1;
            end loop;
            declare
               A : Natural := I;
               Z : Natural := J - 1;
            begin
               while A <= Z and then F (A) = ' ' loop
                  A := A + 1;
               end loop;
               while Z >= A and then F (Z) = ' ' loop
                  Z := Z - 1;
               end loop;
               declare
   ...
   begin
      if B.Kind = Disk then
         if B.Mode /= In_File and then B.FS /= null then
            B.FS.Commit;
         end if;
      end if;
      B.Kind := Closed;
   ...
   begin
      Peek (B, C, Av);
      if Av and then C = ASCII.LF then
         Advance (B);
      end if;
   ...
   package body Fixed_IO is
      ...
      begin
         ...
         declare
            Int_Len : constant Natural := Dot - S'First;
         begin
            if Fore > Int_Len then
               Write_Tracked (B, Spaces (Fore - Int_Len) & S);
            else
