...
package body ESP32S3.Ext4.Dir is
   ...
   generic
      with
        function Visit
          (Name : String; Ino : Inode_Number; File_Type : U8) return Boolean;
         ...
         begin
            if Phys /= 0 then
               while Pos + 8 <= BS loop
                  ...
                  begin
                     exit when
                       Rec_Len < 8;            --  malformed: avoid a loop
                     exit when
                       Pos + Rec_Len > BS;     --  entry must stay in-block
                     if Ino /= 0
                       and then Name_Len > 0
                     ...
                     then
                        ...
                        begin
                           ...
                           if Visit (To_String (Nm), Inode_Number (Ino), Ftype)
                           then
                              return;
                           end if;
                        end;
   ...
   is
      ...
      function Match (Nm : String; Ino : Inode_Number; Ft : U8) return Boolean
      is
   ...
   is
      function Call (Nm : String; Ino : Inode_Number; Ft : U8) return Boolean
      is
      begin
         Visit (Nm, Ino, Ft);
            ...
            if Phys /= 0 then
               while Pos + 8 <= BS loop
                  ...
                  begin
                     exit when Rec < 8;
                     exit when
                       Pos + Rec > BS;          --  entry must stay in-block
                     if Rec - Actual >= Needed then
                        if Ino /= 0 then
                           Put_U16
                             (Hdr, 4, U16 (Actual));      --  shrink current
            ...
            if Phys /= 0 then
               while Pos + 8 <= BS loop
                  ...
                  declare
                     ...
                     NLen : constant Natural := Natural (Get_U8 (Hdr, 6));
                     Nm   :
                       Byte_Array (0 .. (if NLen = 0 then 0 else NLen - 1));
                  begin
                     exit when Rec < 8;
                     exit when
                       Pos + Rec > BS;          --  entry must stay in-block
   ...
   is
      ...
      function Check (Nm : String; Ino : Inode_Number; Ft : U8) return Boolean
      is
