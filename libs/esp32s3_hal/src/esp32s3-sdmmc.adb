...
package body ESP32S3.SDMMC is
   ...
   type Sig_Set is record
      ...
      Ccmd :
        Natural;                 --  command line (out = in, bidirectional)
      Cdat : Dat_Sigs;                --  data lines D0..D3 (out = in)
   end record;

   Sig : constant array (Slot) of Sig_Set :=
     (Slot1 =>
        (Cclk => Sigs.SDHOST_CCLK_OUT_1,
         Ccmd => Sigs.SDHOST_CCMD_OUT_1,
         Cdat =>
           (Sigs.SDHOST_CDATA_OUT_10,
            Sigs.SDHOST_CDATA_OUT_11,
            Sigs.SDHOST_CDATA_OUT_12,
            Sigs.SDHOST_CDATA_OUT_13)),
      Slot2 =>
        (Cclk => Sigs.SDHOST_CCLK_OUT_2,
         Ccmd => Sigs.SDHOST_CCMD_OUT_2,
         Cdat =>
           (Sigs.SDHOST_CDATA_OUT_20,
            Sigs.SDHOST_CDATA_OUT_21,
            Sigs.SDHOST_CDATA_OUT_22,
            Sigs.SDHOST_CDATA_OUT_23)));

   function Card_No (S : Slot) return Natural
   is (Slot'Pos (S));
   ...
   Data_Err      : constant UInt32 :=
     Int_DCRC or Int_DRTO or Int_HTO or Int_FRUN or Int_EBE;
   ...
   type RINTSTS_Bits is record
      Resp_Err, Cmd_Done, Data_Over, RCRC, DCRC, RTO, DRTO, HTO, FRUN, EBE :
        Boolean;
   end record;
   for RINTSTS_Bits use
     record
   ...
   CIU_Span     : constant Ada.Real_Time.Time_Span :=
     Ada.Real_Time.Milliseconds (50);
   Cmd_Span     : constant Ada.Real_Time.Time_Span :=
     Ada.Real_Time.Milliseconds (250);
   Data_Span    : constant Ada.Real_Time.Time_Span :=
     Ada.Real_Time.Milliseconds (1000);
   Busy_Span    : constant Ada.Real_Time.Time_Span :=
     Ada.Real_Time.Milliseconds (1000);
   Reg_Span     : constant Ada.Real_Time.Time_Span :=
     Ada.Real_Time.Milliseconds (50);
   ...
   function Past (Deadline : Ada.Real_Time.Time) return Boolean
   is (Ada.Real_Time.Clock >= Deadline);
   ...
   begin
      ...
      SDHOST_Periph.CLKENA :=
        (CCLK_ENABLE => CLKENA_CCLK_ENABLE_Field (2**Slot_No),
         LP_ENABLE   => 0,
         --  keep the clock running when idle
         others      => <>);
      Clock_Command (Slot_No);
   end Set_Card_Clock;
   ...
   function Addr_Of (C : Card; LBA : Block_Address) return UInt32
   is (if C.Block_Addressed then UInt32 (LBA) else UInt32 (LBA) * Block_Bytes);

   --  Decode the 8-byte SCR (Buf big-endian) into spec version + bus widths.
   procedure Decode_SCR (C : in out Card; Buf : Small_Buf) is
      Sd_Spec  : constant Natural :=
        Natural (Buf (0) and 16#0F#);   --  [59:56]
      Sd_Spec3 : constant Boolean := (Buf (2) and 16#80#) /= 0;      --  [47]
      Sd_Spec4 : constant Boolean := (Buf (2) and 16#04#) /= 0;      --  [42]
      Sd_Specx :
        constant Natural :=                                 --  [41:38]
          Natural
               (Shift_Left (Buf (2) and 16#03#, 2)
                or Shift_Right (Buf (3), 6));
   begin
      ...
      case Sd_Spec is
         ...
         when 2      =>
            if not Sd_Spec3 then
               C.Spec_Major := 2;
            elsif Sd_Specx >= 1 then
               C.Spec_Major := 4 + Sd_Specx;  --  5/6/7..
            elsif Sd_Spec4 then
               C.Spec_Major := 4;
            else
               C.Spec_Major := 3;
   ...
   begin
      ...
      St :=
        Issue (9, Shift_Left (UInt32 (C.RCA), 16), Long_Resp, Slot_No => N);
      C.CSD :=
        (Unsigned_32 (R0),
         Unsigned_32 (R1w),
         Unsigned_32 (R2w),
      ...
      St :=
        Issue (7, Shift_Left (UInt32 (C.RCA), 16), Short_Resp, Slot_No => N);
      Wait_Not_Busy;
      ...
      if C.Width = Width_4 then
         St :=
           Issue
             (55, Shift_Left (UInt32 (C.RCA), 16), Short_Resp, Slot_No => N);
         St :=
           Issue (6, 2, Short_Resp, Slot_No => N);           --  bus width = 4
      ...
      begin
         ...
         St :=
           Issue
             (6, 16#00FF_FFF1#, Short_Resp, Dir => Read_Data, Slot_No => N);
         if St = OK and then Read_Small (64, Buf) = OK then
            C.HS_Supported :=
              (Buf (13) and 16#02#) /= 0;  --  group 1 function 1
         ...
         if C.Want_HS and then C.HS_Supported then
            ...
            St :=
              Issue
                (6, 16#80FF_FFF1#, Short_Resp, Dir => Read_Data, Slot_No => N);
            if St = OK and then Read_Small (64, Buf) = OK then
               C.HS_Active :=
                 (Buf (16) and 16#0F#) = 1;   --  selected function
   ...
   begin
      ...
      St :=
        Issue
          (17, Addr_Of (C, LBA), Short_Resp, Dir => Read_Data, Slot_No => N);
      if St /= OK then
         Data := (others => 0);
         Result := St;
         return;
   ...
   begin
      ...
      St :=
        Issue
          (24, Addr_Of (C, LBA), Short_Resp, Dir => Write_Data, Slot_No => N);
      if St /= OK then
         Result := St;
         return;
      end if;
   ...
   begin
      ...
      SDHOST_Periph.FIFOTH :=
        (TX_WMARK                      => 8,
         RX_WMARK                      => 7,
         DMA_MULTIPLE_TRANSACTION_SIZE => 0,
         others                        => <>);
      SDHOST_Periph.RST_N.CARD_RESET := RST_N_CARD_RESET_Field (2**N);
      SDHOST_Periph.CTYPE :=
        (others => <>);    --  start 1-bit; widened at init
   ...
   function Identity (C : Card) return Card_Id is
      function F (Lo, W : Natural) return Unsigned_32
      is (Field (C.CID, Lo, W));
      function Ch (Lo : Natural) return Character
      is (Character'Val (Natural (F (Lo, 8))));
      Id : Card_Id;
   begin
   ...
   begin
      if Structure = 1 then
         ...
         declare
            C_Size : constant Unsigned_64 :=
              Unsigned_64 (Field (C.CSD, 62, 12));
            ...
            Bytes  : constant Unsigned_64 :=
              (C_Size + 1)
              * Shift_Left (Unsigned_64 (1), C_Mult + 2)
   ...
   function Capabilities (C : Card) return Card_Caps is
      ...
      Mult :
        constant array (0 .. 15) of Natural :=          --  x10 (avoid frac)
   ...
   begin
      ...
      Caps.Read_Block_Len :=
        2**Natural (Field (C.CSD, 80, 4));   --  READ_BL_LEN
