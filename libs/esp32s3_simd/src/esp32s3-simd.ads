...
package ESP32S3.SIMD is
   ...
   type SIMD_I8_Vector is array (Natural range <>) of Integer_8 with Alignment => 16;
   type SIMD_I16_Vector is array (Natural range <>) of Integer_16 with Alignment => 16;
   type SIMD_I32_Vector is array (Natural range <>) of Integer_32 with Alignment => 16;
   type SIMD_F32_Vector is array (Natural range <>) of IEEE_Float_32 with Alignment => 16;
   ...
   procedure Add (A : SIMD_I8_Vector; B : SIMD_I8_Vector; Result : in out SIMD_I8_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Add (A : SIMD_I16_Vector; B : SIMD_I16_Vector; Result : in out SIMD_I16_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Add (A : SIMD_I32_Vector; B : SIMD_I32_Vector; Result : in out SIMD_I32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Add (A : SIMD_F32_Vector; B : SIMD_F32_Vector; Result : in out SIMD_F32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;
   ...
   procedure Sub (A : SIMD_I8_Vector; B : SIMD_I8_Vector; Result : in out SIMD_I8_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Sub (A : SIMD_I16_Vector; B : SIMD_I16_Vector; Result : in out SIMD_I16_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Sub (A : SIMD_I32_Vector; B : SIMD_I32_Vector; Result : in out SIMD_I32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Sub (A : SIMD_F32_Vector; B : SIMD_F32_Vector; Result : in out SIMD_F32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;
   ...
   procedure Mul_Shift
     (A : SIMD_I8_Vector; B : SIMD_I8_Vector; Result : in out SIMD_I8_Vector; Shift : Shift_I8)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Mul_Shift
     (A : SIMD_I16_Vector; B : SIMD_I16_Vector; Result : in out SIMD_I16_Vector; Shift : Shift_I16)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Mul_Shift
     (A : SIMD_I32_Vector; B : SIMD_I32_Vector; Result : in out SIMD_I32_Vector; Shift : Shift_I32)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Mul_Shift
     (A : SIMD_F32_Vector; B : SIMD_F32_Vector; Result : in out SIMD_F32_Vector; Shift : Natural)
   ...
   --    Integer_8  Ã Integer_8  â Integer_16  (simd_mul_i8_to_i16)
   --    Integer_16 Ã Integer_16 â Integer_32  (simd_mul_i16_to_i32)
   --  -----------------------------------------------------------------------

   procedure Mul_Widen (A : SIMD_I8_Vector; B : SIMD_I8_Vector; Result : in out SIMD_I16_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Mul_Widen (A : SIMD_I16_Vector; B : SIMD_I16_Vector; Result : in out SIMD_I32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   --  -----------------------------------------------------------------------
   --  Saturating element-wise negation
   --    Result(i) := -A(i)  (saturates: e.g. Integer_8 -128 â 127)
   --  -----------------------------------------------------------------------
   ...
   procedure Max (A : SIMD_I8_Vector; B : SIMD_I8_Vector; Result : in out SIMD_I8_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Max (A : SIMD_I16_Vector; B : SIMD_I16_Vector; Result : in out SIMD_I16_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Max (A : SIMD_I32_Vector; B : SIMD_I32_Vector; Result : in out SIMD_I32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Max (A : SIMD_F32_Vector; B : SIMD_F32_Vector; Result : in out SIMD_F32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;
   ...
   procedure Min (A : SIMD_I8_Vector; B : SIMD_I8_Vector; Result : in out SIMD_I8_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Min (A : SIMD_I16_Vector; B : SIMD_I16_Vector; Result : in out SIMD_I16_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Min (A : SIMD_I32_Vector; B : SIMD_I32_Vector; Result : in out SIMD_I32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Min (A : SIMD_F32_Vector; B : SIMD_F32_Vector; Result : in out SIMD_F32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;
   ...
   procedure Compare_GT (A : SIMD_I8_Vector; B : SIMD_I8_Vector; Result : in out SIMD_I8_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Compare_GT (A : SIMD_I16_Vector; B : SIMD_I16_Vector; Result : in out SIMD_I16_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Compare_GT (A : SIMD_I32_Vector; B : SIMD_I32_Vector; Result : in out SIMD_I32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Compare_LT (A : SIMD_I8_Vector; B : SIMD_I8_Vector; Result : in out SIMD_I8_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Compare_LT (A : SIMD_I16_Vector; B : SIMD_I16_Vector; Result : in out SIMD_I16_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Compare_LT (A : SIMD_I32_Vector; B : SIMD_I32_Vector; Result : in out SIMD_I32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Compare_EQ (A : SIMD_I8_Vector; B : SIMD_I8_Vector; Result : in out SIMD_I8_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Compare_EQ (A : SIMD_I16_Vector; B : SIMD_I16_Vector; Result : in out SIMD_I16_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Compare_EQ (A : SIMD_I32_Vector; B : SIMD_I32_Vector; Result : in out SIMD_I32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;
   ...
   procedure Bitwise_And (A : SIMD_I8_Vector; B : SIMD_I8_Vector; Result : in out SIMD_I8_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Bitwise_And
     (A : SIMD_I16_Vector; B : SIMD_I16_Vector; Result : in out SIMD_I16_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Bitwise_And
     (A : SIMD_I32_Vector; B : SIMD_I32_Vector; Result : in out SIMD_I32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Bitwise_Or (A : SIMD_I8_Vector; B : SIMD_I8_Vector; Result : in out SIMD_I8_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Bitwise_Or (A : SIMD_I16_Vector; B : SIMD_I16_Vector; Result : in out SIMD_I16_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Bitwise_Or (A : SIMD_I32_Vector; B : SIMD_I32_Vector; Result : in out SIMD_I32_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;

   procedure Bitwise_Xor (A : SIMD_I8_Vector; B : SIMD_I8_Vector; Result : in out SIMD_I8_Vector)
   with Pre => A'Length = B'Length and then A'Length = Result'Length;
   ...
   --  Widening: Integer_8âInteger_16, Integer_8âInteger_32, Integer_16âInteger_32
   --  Narrowing: Integer_16âInteger_8, Integer_32âInteger_8, Integer_32âInteger_16
   --  In all cases Result must have the same number of elements as A.
   --  -----------------------------------------------------------------------

   procedure Convert (A : SIMD_I8_Vector; Result : in out SIMD_I16_Vector)
   with Pre => A'Length = Result'Length;

   procedure Convert (A : SIMD_I8_Vector; Result : in out SIMD_I32_Vector)
   with Pre => A'Length = Result'Length;

   procedure Convert (A : SIMD_I16_Vector; Result : in out SIMD_I32_Vector)
   with Pre => A'Length = Result'Length;
