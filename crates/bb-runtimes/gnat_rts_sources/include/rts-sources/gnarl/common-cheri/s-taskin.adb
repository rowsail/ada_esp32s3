...
package body System.Tasking is
   ...
   is
      ...
      package CHERI renames Interfaces.CHERI;
      package SSE   renames System.Storage_Elements;

      use type CHERI.Bounds_Length;
      use type SSE.Integer_Address;

      Adjusted_Stack_Size : constant System.Parameters.Size_Type :=
                              Parameters.Adjust_Storage_Size (Stack_Size);

      Aligned_Stack_Address : SSE.Integer_Address;
      Stack_Bounds_Size     : CHERI.Bounds_Length :=
                                CHERI.Bounds_Length (Adjusted_Stack_Size);
   ...
   begin
      ...
      --  Align the stack upwards to ensure that the CHERI lower bound is
      --  representable.

      Aligned_Stack_Address :=
        CHERI.Align_Address_Up
          (SSE.To_Integer (Stack_Address), Stack_Bounds_Size);

      Stack_Bounds_Size :=
        Stack_Bounds_Size - CHERI.Bounds_Length
                              (Aligned_Stack_Address
                               - SSE.To_Integer (Stack_Address));

      --  Align the size down so that the CHERI upper bound will also be
      --  representable.

      Stack_Bounds_Size :=
        CHERI.Bounds_Length
          (SSE.Integer_Address (Stack_Bounds_Size)
           and CHERI.Representable_Alignment_Mask (Stack_Bounds_Size));

      --  Mint a capability with the aligned address and bounds

      T.Common.Compiler_Data.Pri_Stack_Info.Start_Address :=
        CHERI.Capability_With_Address_And_Exact_Bounds
          (Cap     => Stack_Address,
           Address => Aligned_Stack_Address,
           Length  => Stack_Bounds_Size);

      T.Common.Compiler_Data.Pri_Stack_Info.Size :=
        Storage_Elements.Storage_Offset (Stack_Bounds_Size);
   end Initialize_ATCB;
