...
\lstcap{Example 1: memory-to-memory copy.}
\begin{lstlisting}
with Interfaces;
...
\lstcap{Example 1: 20\,kHz at 25\,\% on one pin.}
\begin{lstlisting}
...
\lstcap{Example 2: a complementary pair with 500\,ns dead-time (half-bridge).}
\begin{lstlisting}
with ESP32S3.MCPWM; use ESP32S3.MCPWM;
...
\lstcap{Example 1: play a buffer of samples.}
\begin{lstlisting}
...
\lstcap{Example 2: wiring-free full-duplex loopback self-test.}
\begin{lstlisting}
with ESP32S3.I2S; use ESP32S3.I2S;
...
\lstcap{Example 1: transmit a pulse burst (1\,\textmu s resolution).}
\begin{lstlisting}
...
\lstcap{Example 2: receive into a buffer (e.g. an IR remote).}
\begin{lstlisting}
with ESP32S3.RMT; use ESP32S3.RMT;
procedure RMT_Rx is
   Rx   : RX_Channel;
...
\lstcap{Example 1: count rising edges on a pin.}
\begin{lstlisting}
...
\lstcap{Example 2: count both edges (double the resolution).}
\begin{lstlisting}
with ESP32S3.PCNT; use ESP32S3.PCNT;
...
\lstcap{Example 1: a 25\,\% density bitstream (analog-ish level via an RC filter).}
\begin{lstlisting}
...
\lstcap{Example 2: breathe an LED by sweeping density.}
\begin{lstlisting}
with Ada.Real_Time; use Ada.Real_Time;
with ESP32S3.SDM;    use ESP32S3.SDM;
...
\lstcap{Example 1: send a standard and an extended frame.}
\begin{lstlisting}
...
\lstcap{Example 2: self-test loopback (no transceiver, no wiring).}
\begin{lstlisting}
with ESP32S3.TWAI; use ESP32S3.TWAI;
...
\lstcap{Example 1: measure an elapsed interval in microseconds.}
\begin{lstlisting}
with Ada.Text_IO;   use Ada.Text_IO;
...
\lstcap{Example 2: a one-shot alarm 50\,ms out.}
\begin{lstlisting}
with ESP32S3.Timer; use ESP32S3.Timer;
...
\lstcap{Example 1: push a frame buffer to a parallel display.}
\begin{lstlisting}
with Interfaces;
...
\lstcap{Example 2: expose the pixel clock on a pad (for a scope).}
\begin{lstlisting}
with ESP32S3.LCD; use ESP32S3.LCD;
...
\lstcap{Example 1: read a 12-bit sample.}
\begin{lstlisting}
with Ada.Text_IO; use Ada.Text_IO;
...
\lstcap{Example 2: a lower attenuation for a small signal.}
\begin{lstlisting}
with Ada.Text_IO; use Ada.Text_IO;
with ESP32S3.ADC; use ESP32S3.ADC;
