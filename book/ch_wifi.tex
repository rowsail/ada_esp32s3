...
\section{The storage the driver does not have}\index{Wi-Fi!NVS}\index{NVS}\index{Wi-Fi!RF calibration}
\label{ch:wifi:nvs}

ESP-IDF's Wi-Fi stack expects a key/value store in flash, called NVS. Porting it
looked at first like part of the work. It is not here, and leaving it out was a
decision rather than a shortcut.

NVS earns its place in ESP-IDF for two jobs. The first is caching the PHY
calibration blob. Calibration tunes the radio to this particular chip and
antenna, it takes tens of milliseconds, and it runs on every bring-up. Cache the
result and the next boot can do a fast partial calibration instead. The second
job is remembering the network: \texttt{esp\_wifi\_set\_storage} can persist an
SSID and passphrase across reboots.

This port does neither. The \texttt{wifi\_init\_config\_t} it hands the blob sets
\lstinline{Nvs_Enable := 0}, so the blob never looks for a store, and nothing
calls \texttt{esp\_wifi\_set\_storage}, so the configuration lives in RAM and is
set on each boot.

\cnote{In an ESP-IDF project you call \texttt{nvs\_flash\_init()} before
\texttt{esp\_wifi\_init()}, and a missing or full NVS partition is a familiar
early-boot failure. Nothing here needs that call, and there is no partition to
get wrong.}

The calibration capability does not disappear with the store, though. It moves
to the application through a pair of hooks:

\begin{lstlisting}
procedure Set_Cal_Store (Load : Cal_Load_Hook; Store : Cal_Store_Hook);
\end{lstlisting}

The driver keeps the decision it is qualified to make, which is whether a full or
a partial calibration is required. The application supplies the medium. That may
be a flash partition, the external SPI NOR of Chapter~\ref{ch:storage}, or
nothing at all, in which case the radio simply calibrates fully on every boot.

That seam is the point. A driver has no business knowing how a particular board
persists bytes, and NVS would have bound this one to a single answer. The blob it
hands over is opaque, 1904 bytes, and carries its own version number and the
chip's MAC address. The driver re-checks both before trusting a blob it is given,
so an image moved to a second board recalibrates rather than tuning itself with
another chip's numbers.

The same reasoning retired a binary. Espressif's \texttt{libcore.a} was linked
only for a handful of symbols behind its misc-NVS code, which is dormant once NVS
is off. \texttt{ESP32S3.WiFi.Core\_Shim} provides those symbols in Ada, and the
blob count drops by one.

What remains in the OS adapter is a boundary marker. All twelve NVS slots are
bound to named stubs that halt and print which call was attempted, so a request
for \texttt{nvs\_set\_blob} says exactly that. They are not a list of unfinished
work: reaching one would mean a code path we do not exercise has started wanting
persistent storage. The adapter table carries no \lstinline{others} clause at
all. Every one of its 120 slots is named, so adding a slot to the record fails to
compile until somebody decides what belongs in it. That is a better guarantee
than a catch-all, which quietly absorbs whatever nobody thought about.

One piece here is genuinely unfinished. The calibration hooks exist, but the only
implementation of them is a demonstration that prints the blob for pasting into a
source constant. Writing it to flash and reading it back would make every boot
after the first a fast partial calibration. The seam is ready for it.
...
free-running 16\,MHz counter that keeps ticking through \texttt{waiti}. Each
core arms one of its comparators. (At the time the comparator's interrupt was
routed to the old level-5 vector; the tick has since moved into the level-2 OS
band --- Chapter~\ref{ch:interrupts}.) Only the timer underneath changed:
...
begin
   --  Arm this core's SYSTIMER comparator (one-shot).  The WORK_EN toggle is
   --  load-bearing: the comparator checks "target already passed?" only on
   --  the WORK_EN rising edge.  See the sequel below.
   Work_En (Core, False);
   Load_Comparator (Core, Deadline);
   Work_En (Core, True);
end Set_Alarm;
\end{lstlisting}
...
\textbf{The sequel: the alarm that armed in the past.} The first version of
this code did not toggle \texttt{WORK\_EN}, and its comment claimed the
comparator ``fires on count \texttt{>=} target, so even a past deadline fires
at once''. The hardware disagrees. Arming takes several register writes, and
the deadline floor is one 16\,MHz tick --- 62.5\,ns. When a burst of timing
events produced a near-zero re-arm delta, the free-running count overtook the
target \emph{during the arming sequence}, and a comparator loaded with a
target already in the past never fires. One missed arm killed the alarm chain
outright: no tick, no expiring delay, every delaying task suspended forever ---
while the pure-interrupt machinery (an LCD scan-out, the CAN controller)
carried on. The result was a live screen full of dead tasks, and it surfaced
deterministically: a J1939 demo froze exactly four seconds after boot, every
boot. The S3 does have hardware compensation for exactly this case
(\texttt{SOC\_SYSTIMER\_ALARM\_MISS\_COMPENSATE}: fire immediately if the
loaded target is already behind the count) --- but it evaluates on the
\texttt{WORK\_EN} rising edge, which is why esp-idf's \texttt{systimer\_hal}
toggles the enable around every load, and why leaving it permanently on
silently defeats the safety net. The sharpened lesson, twice over: read the
vendor's HAL as documentation. The register sequence it performs is part of
the datasheet.
