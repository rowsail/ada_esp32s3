...
One build knob is worth knowing before you need it.\index{BUILD=Assert}
\index{-gnata!runtime} The compile step honours \texttt{BUILD=Assert}, which
compiles the \emph{runtime itself} with assertions enabled
(\texttt{-gnata}, no \texttt{-gnatp}): every internal
\texttt{pragma Assert}, precondition and invariant in GNARL and the kernel
becomes live code. It is slower and bigger, and it is the single most
effective torture test the runtime has \textemdash{} an assertion-enabled RTS
surfaced two latent kernel bugs (a stale base priority and an SMP-unsound
postcondition, \S\ref{ch:gnarl-gnull}) that production builds had silently
tolerated for months. All three profiles build and run this way on hardware,
the full profile included. Develop against it when you touch the runtime;
ship \texttt{Production}.
...
\texttt{s-textio\_\_esp32s3.adb} & console \texttt{Put}: a settable hook (the HAL routes it to \texttt{ESP32S3.Serial}), else the ROM \texttt{esp\_rom\_printf} \\
...
\texttt{Finalize\_Protection} & supply the protected-object finalization hook
  the full compiler expects (a no-op: the bareboard lock holds no dynamic
  state) \\
elaboration-order idle repair & rebuild the boot CPU's idle ATCB after this
  package's own body elaboration wipes it (below) \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{A hook that runs before elaboration}\index{elaboration order}\index{Runtime\_Initialize}

The last patch in the table earns a walkthrough, because the defect class is
easy to reintroduce.  The binder's generated \texttt{adainit} calls
\texttt{\_\_gnat\_runtime\_initialize} \emph{before the first library unit
elaborates}, and on the full runtime that hook reaches
\texttt{System.Tasking.Initialize}: exception occurrences need the task layer
(thread-local storage) from the very first raise, which can happen during
elaboration itself.  \texttt{Initialize} builds the boot CPU's idle-task ATCB
\textemdash{} and then, much later in \texttt{adainit}, the elaboration of
\texttt{System.Task\_Primitives.Operations}' \emph{own body}
default-initialises the library-level \texttt{Idle\_Tasks} array and quietly
re-defaults the ATCB that is already in service.  The wreckage is deferred and
misleading: the idle thread's \texttt{LL.Thread} link is null again, and the
first interrupt handler that takes a protected object's ceiling lock
dereferences it \textemdash{} a \texttt{LoadProhibited} at offset
\texttt{16\#38\#}, minutes of call chain away from the cause.

No elaboration pragma can fix this, because the offending call is outside the
elaboration graph entirely.  The patch does two things:
\texttt{pragma Elaborate\_Body} on the spec pins every \emph{Ada} client
after the body's elaboration, and a \texttt{Repair\_Idle\_After\_Elaboration}
procedure \textemdash{} called from \texttt{System.Tasking.Initialization}'s
elaboration, the first point after the wipe \textemdash{} rebuilds the idle
task.  Rebuilding is safe precisely because the idle thread cannot have run
before elaboration completes: the environment task has not blocked yet.  The
moral for porters: on the full runtime, treat any library-level object that a
binder-driven hook may touch as \emph{hostile to its own elaboration}, and
re-establish it afterwards rather than trying to order around the binder.
...
\begin{quote}
\lstcap{After editing a generated runtime \texttt{.adb}, delete the archives
before rebuilding:}
\begin{lstlisting}[style=sh]
