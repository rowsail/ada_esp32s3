...
\section*{The open edges}

The project is not finished, and a few edges are left deliberately visible. A
cellular modem driver is built but held until improved hardware. The chip's internal
flash as an ext4 volume is scoped but not written. The capstone that would wire a
sensor, the filesystem, and the TLS client into one logger is sketched but not yet a
running example. More cipher suites, a deeper treatment of the two-core scheduler,
and a fuller low-power story all have room to grow. These are not apologies. They are
the next chapters, and the repository is where they will appear.

\section*{Where to go from here}

Which is the last point. The book is a companion to a living code base, not a closed
artifact. Every concept here is anchored in code you can clone, build, and run with
