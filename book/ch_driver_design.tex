...
it is a singleton. A UART fills the same record with adapters whose \texttt{Ctx}
is the address of a held \texttt{UART.Session}:
...
\texttt{ESP32S3.Log} writes through the mux, and the runtime's
\texttt{System.Text\_IO} is bridged into it by a single installed hook, so
\emph{both} the HAL logger and \texttt{Ada.Text\_IO} follow \texttt{Set\_Output}
together. The pattern shows up a third time inside the console itself: its
drop-notification hook, \texttt{On\_Drop}, is one more library-level
\texttt{access procedure (Count : Natural)} the application supplies.
