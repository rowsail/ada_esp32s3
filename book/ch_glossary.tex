...
\item[ACK polling] Probing an I2C device's address until it answers. An EEPROM stops acknowledging while it programs a page, so the master polls instead of sleeping the worst case (Section~\ref{storage:eeprom}).
...
\item[EEPROM] \emph{Electrically Erasable Programmable Read-Only Memory} -- small, byte-addressable non-volatile memory needing no erase before a write. The 24C family speaks I2C (Section~\ref{storage:eeprom}).
