...
\begin{itemize}
  \item \textbf{Precise stack-overflow detection \textemdash{} and recovery.}
        Every profile arms a hardware watchpoint a redzone above each task's
        stack limit as the task starts, so an overflow write faults
        \emph{precisely} (a debug exception) instead of silently corrupting a
        neighbour.  On the \texttt{full} and \texttt{embedded} profiles the
