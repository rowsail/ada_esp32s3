...
\begin{itemize}
  \item \textbf{Precise stack-overflow detection \textemdash{} and recovery.}
        Every profile guards the \emph{running} task with a hardware
        watchpoint a redzone above its stack limit: the context switch
        re-arms it for the incoming thread on every switch (cooperative and
        preemptive alike), so an overflow write faults \emph{precisely} (a
        debug exception) instead of silently corrupting a neighbour \textemdash{}
        whichever task overflows, whenever it runs.  On the \texttt{full} and \texttt{embedded} profiles the
        build additionally links the recovery path, which turns the trip into
        a \texttt{Storage\_Error} raised in the faulting task and caught by
        an ordinary \texttt{when Storage\_Error} handler \textemdash{}
        hardware-verified by driving a task into deliberate recursive overflow
        and watching its handler run, with the task alive afterwards.
        \texttt{light-tasking} stays detection-only: a trip parks in the
        panic breadcrumb dump.

        One constraint follows for \emph{diagnostic tools}: the watched
        redzone sits 2\,KB above each task's stack limit, and any tool that
        writes near the limit trips it.  A stack high-water monitor that
        paints the unused stack with a sentinel must start its paint (and its
        scan) \emph{above} the redzone plus the 64-byte watch window \textemdash{}
        paint from the very limit and the monitor itself raises the
        \texttt{Storage\_Error} it set out to measure.  Nothing is lost by
        stopping short: a task that ever reached the redzone would take
        \texttt{Storage\_Error} anyway, so the usable stack \emph{is} what
        lies above it.  (The engine-monitor example's run-statistics harness
        is a worked example of both the technique and the trap.)

        The recovery path has one design constraint worth recording: the
        debug-exception handler cannot simply call the raise through an
        assembly trampoline, because Xtensa \texttt{gas} cannot emit CFI and
        an FDE-less frame aborts the ZCX unwinder's walk \textemdash{} the
        exception would never reach its handler.  Instead the handler
        \emph{emulates a \texttt{call4}} from the faulting instruction into
        the runtime's raise procedure (sets the call4-encoded return address
        and \texttt{PS.CALLINC}, then \texttt{rfi}), so the unwinder only
        ever walks real, compiler-described Ada frames.  The redzone is
        2\,KB: the raise and the unwinder's own machinery run on the faulting
        stack \emph{below} the watched line before recovery lands.
...
Three decoding traps, each paid for in hours.  The double-exception vector
...
\emph{double exception}, not \emph{instruction fetch error}.  PRID does
...
The third trap is the most seductive: a halted PC is not always a fault.  The
runtime parks \emph{deliberately} in several places \textemdash{} the panic
park above, and the interrupt vectors' stack sanity net
(\S\ref{ch:interrupts}), which parks with its evidence in
\texttt{\_\_gnat\_bad\_sp}.\index{\_\_gnat\_bad\_sp}  At a deliberate park the
special registers are \emph{stale}: EXCCAUSE and EPC1 still hold whatever the
last handled exception left there, and on the sanity-net path the vector has
even reused \texttt{sp} to hold the interrupted PC.  Read those registers as
if they described a live fault and they will tell you a detailed, coherent,
wrong story.  We once diagnosed ``register-window corruption on PSRAM
stacks'' from exactly such a read \textemdash{} stale EXCCAUSE, a
flash-address ``SP'' \textemdash{} when the truth was the sanity net itself
parking on a then-unrecognised (and perfectly valid) PSRAM stack address.  So
before decoding anything: symbolise the halted PC.  If it sits on a park loop,
identify \emph{which} park, and trust only the breadcrumbs that park wrote
deliberately.
