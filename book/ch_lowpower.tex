...
powered and only the radio idles between activity. There the RF \emph{must} be powered
down explicitly and repeatedly (\texttt{phy\_close\_rf}), and the lower-MAC blob drives
exactly that: it toggles the PHY off and on roughly once a second as its own power
management, through the \texttt{phy\_enable} / \texttt{phy\_disable} hooks the OS adapter
provides (Chapter~\ref{ch:wifi}). The adapter \emph{honours} them --- \texttt{phy\_disable}
saves the PHY digital registers, closes the RF and powers down the temperature sensor;
\texttt{phy\_enable} re-wakes the PHY and restores those registers --- so the radio
really powers down between activity even while the core runs, not only in deep sleep.
Deep sleep needs none of that cooperation, because it does not leave the domain powered
to begin with.
