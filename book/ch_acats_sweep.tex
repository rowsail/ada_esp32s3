...
\smallskip
\textbf{Superseded figures.} The counts in this chapter are those of the original
Python sweep described here. The suite is now driven by a separate harness
(\texttt{rowsail/Ada\_esp32s3\_acats}: a filter/producer/consumer pipeline that
pulls this repository in as a submodule), and \emph{its} results are the current
conformance evidence --- see \texttt{ACATS\_RESULTS.md} there, and the summary
table in this repository's \texttt{README.md}. The methodology below --- one test
per image, why bundling loses passes, how grades are parsed --- is unchanged and is
why this chapter is still here.}}
\medskip
...
\texttt{Interrupt\_Priority} task runs with the kernel tick's level masked),
