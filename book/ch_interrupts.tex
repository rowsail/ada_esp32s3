...
\texttt{Device\_L3\_Priority}. Two riders on that list. First,
\texttt{Device\_L2\_0} already has an owner: \texttt{ESP32S3.Shared\_L2}, one
protected object that dispatches every registered service on each interrupt.
The HAL's UART and TWAI receivers register with it, so a driver that wants
level~2 should call \texttt{Shared\_L2.Register} rather than attach --- Ada
allows one handler per interrupt, and a second \texttt{Attach\_Handler} is a
\texttt{Program\_Error} at elaboration, which on a board means a boot
loop.\index{Shared\_L2} Second, CPU interrupt 21 is the kernel's: the systimer
alarm and the cross-core poke share it. It once had a name here, read as a free
slot, and cost days. The name is gone --- claiming it is now a compile error
--- and the runtime refuses a raw-literal escape at elaboration with a message
that says whose interrupt it is.
...
\begin{itemize}
  ...
  \item \textbf{Not every CPU interrupt is usable.} The 32 slots are a mixed
        ...
        \texttt{Ada.Interrupts.Names} is as tight as it is. Two traps look
        free on paper. CPU interrupt 29 is an internal software interrupt the
        matrix cannot drive. And CPU interrupt 22 is \emph{edge}-triggered ---
        the one edge slot at level 3 --- which is worse than useless for a
        matrix source. A peripheral asserts a level and holds it; the edge
        latches a pending bit that only an explicit \texttt{INTCLEAR}
        retires, so the handler acknowledges its peripheral, returns, and is
        re-entered at once, forever. No task runs again, no fault is raised,
        and every peripheral register reads healthy. Measured, at the cost of
        a day: the runtime now refuses 22 by name at elaboration.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Stack sanity net.}\index{\_\_gnat\_bad\_sp} Before touching
        the interrupted stack, the vector checks that the interruptee's SP
        lies in a plausible stack region: internal DRAM or the PSRAM window
        (full-profile task stacks may live in either,
        \S\ref{ch:static-mem}). A wild SP means the context was corrupted
        \emph{before} this interrupt, and building the save frame through it
        would scribble whatever it points at. The vector instead parks the
        core, fully masked, with the bad SP and the interrupted PC preserved
        in \texttt{\_\_gnat\_bad\_sp} (see the park-decoding traps in
        \S\ref{ch:debug}).
...
\subsection{The ceiling protocol is an invariant, not a courtesy}\index{ceiling protocol}\index{Leave\_Kernel}\index{poke}

The wrapper's priority raise in step~5 is easy to read as bookkeeping for
the handler's benefit.  It is load-bearing for the \emph{vector}: every
kernel section ends with \texttt{Leave\_Kernel}, and \texttt{Leave\_Kernel}
re-enables interrupts to the \emph{running thread's active priority}.  With
the raise in place that maps back into the interrupt band and the mask
holds; without it, the first task wakeup inside a handler drops
\texttt{INTLEVEL} to the interrupted task's band \emph{in the middle of the
vector}, re-opening exactly the OS-band nesting the architecture above
forbids.

This port learned that the hard way.  The cross-core \emph{poke} (the IPI
that runs a CPU's deferred wakeups) shares the level-2 slot with the tick
and was originally serviced \emph{before} \texttt{Interrupt\_Wrapper},
outside the ceiling -- so under a cross-core wakeup storm the system spent
its life in the forbidden nested regime, and the symptom was intermittent,
maddeningly untraceable register corruption in interrupt-context code (a
stress suite reproduced in eight seconds what months of application soaking
never hit; the fix is eleven lines wrapping the poke in the same
\texttt{Change\_Priority} pair the wrapper uses).  The rule it bought:
\textbf{anything that can reach \texttt{Enter\_Kernel}/\texttt{Leave\_Kernel}
from inside a vector must run under the interrupt's ceiling} -- there is no
harmless amount of kernel work outside it.

There is a second edge to the same invariant: the ceiling must be raised
\emph{high enough}.  Wrapping the poke in \texttt{Change\_Priority} fixed
\emph{where} the raise happens, but it raised to the poke slot's own level-2
priority -- correct only while level~2 was the highest interrupt in play.
Once the RGB-LCD bounce refill was promoted to a level-3 source (to beat its
\textasciitilde1\,ms deadline), the \texttt{Leave\_Kernel} inside
\texttt{Poke\_Handler} re-enabled interrupts to that level-2 band and left the
level-3 refill \emph{unmasked}; it preempted the ceiling-restore
\texttt{Change\_Priority} mid-requeue and self-looped a CPU's ready queue
(\texttt{idle.Next := idle}), freezing the core -- the panel showed one
repeated strip as \texttt{LCD\_CAM} kept cycling a stale bounce buffer.  The
corrected ceiling is \texttt{Interrupt\_Priority'Last}, masking \emph{every}
OS-band interrupt across the poke.  The sharpened rule: \textbf{a from-vector
ceiling must dominate the highest interrupt level that can fire, not merely
the level being serviced.}  A dedicated stressor now fires a software
level-3 interrupt at \textasciitilde25\,kHz beneath the cross-core ping-pong
to hold that regression shut.
...
A third, lighter trigger sits alongside these. \texttt{SW\_L3}
(\texttt{CPU\_INT}~29) is a \emph{software} interrupt at level~3 --
\texttt{wsr.intset} makes it pending and the handler acks with
\texttt{wsr.intclear} -- so, unlike the external device slots (23/27, driven
by the interrupt matrix), it needs no source at all: any code can synthesize
a level-3 interrupt on demand, at whatever rate it likes. That makes it the
cleanest way to \emph{manufacture} an OS-band coincidence for testing ---
\texttt{esp32s3\_stress} fires it at \textasciitilde25\,kHz to land a level-3
interrupt inside the cross-core poke window and hold the ceiling fix above
shut. The same primitive generalizes to escalating work to interrupt
priority from a lower context: set the bit and the handler runs preemptively,
with bounded latency, the instant \texttt{INTLEVEL} permits -- the
self-directed counterpart to the cross-core poke, which signals the
\emph{other} core.
