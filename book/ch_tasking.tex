...
it up at elaboration, with no run-time call (\S\ref{ch:registers} and the Interrupts
chapter):
...
\begin{lstlisting}
Guard : Natural := 0;
while not Done (C, Dir) and then Guard < 50_000_000 loop  -- poll GDMA's Done
   Guard := Guard + 1;
end loop;
...
expected wait is short), but liveness is guaranteed. The I2S driver's
\texttt{Capture} (which polls GDMA's \texttt{Done} directly so it can sample while
a continuous transmit drives the shared clock) and the W5500's command-accept
(\texttt{Issue}, capped at 2000 tries) use this shape.
...
\texttt{ESP32S3.GDMA}'s \texttt{Wait} sits here: a transfer's end-of-transfer
interrupt \texttt{Set\_True}s a per-channel \texttt{Suspension\_Object} and the
waiting task sleeps on it, so a multi-millisecond DMA hands the core back instead
of pinning it. It even folds in the bottom of the spectrum, a brief spin first,
for transfers too short to be worth a context switch, so one \texttt{Wait} spans
three of these four points by the length of the transfer.
...
short, but may never end       & bounded spin (guard count)     & W5500 \texttt{Issue}, GDMA \texttt{Wait}'s pre-spin \\
milliseconds                   & paced poll (\texttt{delay until}) & reset/settle, \texttt{Wait.Until\_True} \\
long / unbounded / power       & block on an interrupt          & GDMA \texttt{Wait}, W5500 \texttt{Wait\_Data}, DRDY \\
\bottomrule
\end{tabular}
\end{center}
...
holds it until something is available, with no polling.
...
the first core, \texttt{2} the second):
...
Omitting the aspect does \emph{not} make a task migratory.  The kernel assigns
every new thread to the core that created it
(\texttt{Initialize\_Thread} sets \texttt{Base\_CPU := This\_CPU}), and it
stays there for life.  Since the environment task elaborates library-level
tasks on core~0, a program that never writes a \texttt{CPU} aspect runs
\emph{entirely} on core~0 while core~1 idles \textemdash{} plainly visible in
the engine-monitor example's run-statistics affinity column
(\S\ref{ch:perf}).  The \texttt{CPU} aspect is therefore not an optimisation
hint; it is the only way work reaches the second core.
...
at once, so funnel diagnostics through one task. With those in mind, an SMP
application is just ordinary tasks, pinned where you want them, sharing through
protected objects.

A shared timer also means \emph{shared control registers}, and those bite in a
subtler way. The \texttt{SYSTIMER\_CONF} register holds the alarm-enable bit for
\emph{both} cores' comparators (\texttt{TARGET0\_WORK\_EN}, \texttt{TARGET1\_WORK\_EN})
in one 32-bit word, so every write to it is a read-modify-write over state the
other core owns half of. The runtime serialises the arming sequence
(\texttt{Set\_Alarm}) behind an \texttt{s32c1i} spinlock for exactly this reason:
two cores re-arming at once would each read the word, flip their own bit, and
write back --- and the second writer would restore a stale copy of the
\emph{first} core's bit, either killing a just-armed comparator (a lost
\texttt{delay until}, permanently) or re-enabling one mid-arm over a half-written
target (a spurious fire that in practice became a self-sustaining
\textasciitilde60\,\textmu s cross-core interrupt storm). The lesson generalises:
if a peripheral packs per-core control into one register, \emph{every} writer must
take the same lock --- including one-time setup code. This port learned it when
core~1's boot-time systimer initialisation, which set its \texttt{WORK\_EN} bit
without the lock, was found racing core~0's already-running \texttt{Set\_Alarm}
during bring-up; the fix was to drop the unlocked init write entirely and let the
first locked \texttt{Set\_Alarm} establish the bit.
