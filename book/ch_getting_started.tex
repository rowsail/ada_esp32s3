...
The \texttt{bb-runtimes} fork that carries the ESP32-S3 bareboard runtime and
\texttt{xtensa-dynconfig} are vendored as ordinary directories, so there are no
submodules to fetch. Either unzip a tagged release archive (no git required) or
clone:
...
\noindent The book itself is attached as a PDF to the same release.

\section{The \texttt{./x} dispatcher}\index{x helper script}

The repository ships with 96 ready-to-run examples, and one script at
...
\texttt{esp32-ada build [-P \textit{prof}]} & Build to \texttt{app.bin}. \textit{prof} = \texttt{light-tasking}, \texttt{embedded}, or \texttt{full}; with no \texttt{-P} it follows the project's \texttt{build.sh} (a fresh \texttt{init} = \texttt{embedded}). \\
...
SDK's \texttt{libs/}. A fresh project defaults to the \texttt{embedded} profile: its
\texttt{build.sh} exports \texttt{ESP32S3\_RTS\_PROFILE}, so change that line to
\texttt{light-tasking} or \texttt{full} to build under another
...
esp32-ada build                                         # the scaffold defaults to embedded
...
\item \textbf{Match the profile.} The scaffold defaults to \texttt{embedded}, which
  is what most examples assume (the HAL's RAII handles need it), so their sources
  drop in and build. An example built for \texttt{light-tasking} (its profile is
  shown by \texttt{./x list} and its \texttt{README}) needs the \texttt{build.sh}
  line \texttt{ESP32S3\_RTS\_PROFILE} set to match.
\item \textbf{Bring the board config if customized.} If the example sets
  a non-default flash/PSRAM size, copy its \texttt{board.ads} (or run
  \texttt{esp32-ada config}). An example may also carry an optional
  \texttt{glue.c} at its root for C natives --- none currently does, the boot
  glue itself being Ada --- and that would need copying too.
