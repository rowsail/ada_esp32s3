...
\item \textbf{Fetch} the new image over HTTPS into the inactive slot, writing it
  ...
  the whole image: a sector at a time, with internal SRAM as the staging
  buffer \textemdash{} flash programming runs with the cache disabled, and
  PSRAM is unreachable while the cache is off, Chapter~\ref{ch:psram}).
