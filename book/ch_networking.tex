...
\textbf{A bounded connect.} \texttt{Connect} is the one operation that must never
wait forever: a peer that is down, or a \texttt{SYN} that vanishes into a dropped
segment, would otherwise hang the calling task indefinitely. The engine gives
\texttt{Connect} a deadline and polls the socket's status register in short
\texttt{delay until} steps until it either reaches \texttt{ESTABLISHED}, transitions
to \texttt{CLOSED} (refused --- reported immediately), or the deadline passes
(\texttt{Timed\_Out}). This is deliberately \emph{not} the interrupt-suspend the
data path uses: a connection that just hangs never raises the \texttt{INTn} that a
suspend would sleep on, so a deadline check sitting behind that suspend is
unreachable --- the bounded poll is what makes the timeout actually fire. Everything
above inherits it, so a \texttt{GNAT.Sockets.Connect\_Socket} to an unreachable host
returns \texttt{Socket\_Error} instead of wedging the task.

\section{Low power: putting the PHY to sleep}\index{W5500!power-down}\index{power saving}
\label{ch:net:lowpower}

The W5500 has exactly one power lever, and it is a blunt one: power the Ethernet
PHY down. That is not a small thing to switch off --- the 100BASE-TX line driver is the bulk
of the chip's current --- so it is the power saving that matters. What it costs you
is the link: with the PHY off there is no carrier, the switch port goes dark, and no
frames move. Everything on the host side of the PHY --- the SPI interface, the MAC,
every register, and the 32\,KB of socket buffers --- stays powered, so your
configuration and any buffered data survive the nap untouched. It is a
\emph{link-layer} sleep, not a chip reset.

The whole mode is one register, \texttt{PHYCFGR}. Its top bit is an active-low PHY
reset; bit~6 says ``take the PHY mode from my register bits, not the hardware
strapping pins''; and a three-bit field selects the mode, of which \texttt{110} is
power-down and \texttt{111} is the normal all-capable auto-negotiating mode. The one
subtlety the datasheet insists on is that a mode change only takes effect when you
\emph{pulse} the reset bit, so the driver writes the new mode, drives reset low,
waits, and releases it. All of that hides behind a two-value enumeration:

\begin{lstlisting}
type Power_Mode is (Normal, Power_Down);

procedure Set_Power (Dev : in out Device; Mode : Power_Mode);
function  Power (Dev : Device) return Power_Mode;   --  reads it back
\end{lstlisting}

\begin{lstlisting}
Net.Set_Power (Dev, Net.Power_Down);   --  PHY off: link drops, chip stays configured
--  ... the low-current interval ...
Net.Set_Power (Dev, Net.Normal);       --  PHY back: the link re-negotiates
--  auto-negotiation takes a moment, so wait for it before using the network:
while Net.Link (Dev) /= Net.Up loop delay until Clock + Milliseconds (250); end loop;
\end{lstlisting}

Because \texttt{Power} reads the mode back out of \texttt{PHYCFGR}, you can confirm
the chip accepted the command even with no cable attached --- which is exactly how
\texttt{esp32s3\_w5500\_lowpower} verifies it on the bench: it cycles the PHY down
and up three times and reads the mode back each time. With a cable in a live switch
you additionally see \texttt{Link} track \texttt{Up}\,$\to$\,\texttt{down}\,$\to$\,\texttt{Up}
and the port LED blink out and back.

\textbf{What sleeps, and what does not.} An open TCP connection does \emph{not}
survive the PHY sleeping --- the link dropping tears it down --- but the socket
\emph{registers} do. So treat this like a brief cable-unplug: sleep the PHY when
idle, and on wake re-establish connections once \texttt{Link} reports \texttt{Up}
again. There is no lighter, still-reachable sleep on the wired PHY --- it is
all-or-nothing --- so a node that must stay reachable keeps the PHY up (waking the
host on \texttt{INTn} instead) rather than sleeping it.

\section{Tuning the chip: the control registers}\index{W5500!control registers}
\label{ch:net:control}

Power is one knob among many. The hardwired stack is configurable through its
registers, and the driver surfaces the useful ones as small, app-controlled calls
in the same style as \texttt{Set\_Power} --- each writes (or reads back) exactly one
register field, nothing implicit. They fall into a few groups.

\textbf{Link.} Beyond auto-negotiation you can pin the PHY:
\texttt{Set\_Link\_Mode (Dev, M10\_Half)} forces 10BASE-T, which draws less than
100BASE-TX --- the companion to power-down for a device that must stay reachable but
sip current --- or fixes a speed/duplex a stubborn switch will not negotiate. Like
\texttt{Set\_Power} it re-inits the PHY, so wait for \texttt{Link} afterward.

\textbf{Mode-register switches.} Three independent behaviours, each off by default:
\texttt{Set\_Wake\_On\_LAN} arms the chip to raise its magic-packet interrupt (the
wired analogue of the server-wake pattern --- keep the PHY up, sleep the host, wake
on the packet); \texttt{Set\_Ping\_Block} stops it auto-answering ICMP echo for a
quieter presence; \texttt{Set\_Force\_ARP} makes it announce its MAC on every send.

\textbf{Retransmission.} \texttt{Set\_Retransmission (Dev, Timeout, Retries)} governs
how long, and how many times, TCP and ARP retry before giving up as
\texttt{Timed\_Out} (default 200\,ms\,$\times$\,8). Shorten it for a snappier
\texttt{Connect} timeout on a dead peer; lengthen it to ride out a lossy link. It is
the one control with an easy on-chip proof --- set it, \texttt{Get} it back --- which
is how \texttt{esp32s3\_w5500\_control} verifies the whole register path on hardware.

\textbf{Per-socket options.} Set on an open \texttt{Socket}:
\texttt{Open\_TCP (\dots, No\_Delay => True)} disables the delayed-ACK batching for
latency-sensitive request/response (Modbus); \texttt{Set\_Keepalive} overrides the
default 60\,s probe (it holds a NAT binding open and detects a vanished peer);
\texttt{Set\_TTL} / \texttt{Set\_Type\_Of\_Service} stamp the IP header;
\texttt{Set\_Max\_Segment\_Size} caps the TCP MSS; and \texttt{Set\_Buffer\_Sizes}
re-allocates this socket's slice of the chip's 16\,KB-per-direction pool --- hand a
bulk transfer 8 or 16\,KB and reclaim it from sockets you do not use (the
per-direction totals must stay within 16\,KB).

\textbf{Diagnostics.} \texttt{Read\_Faults} returns what the common interrupt
register latched: an IP-address conflict (a duplicate of your address appeared on the
LAN) or a destination-unreachable, the latter with the offending IP and port from the
\texttt{UIPR}/\texttt{UPORTR} registers --- so a UDP send that bounced tells you where
it went. \texttt{Set\_Common\_Interrupts} routes any of these (and the magic packet)
to the \texttt{INTn} pin.

\textbf{Multicast and raw frames.} \texttt{Open\_UDP\_Multicast} joins an IPv4 group
(the driver derives the multicast MAC --- one of the few genuine bit-transforms here,
so it is pinned by a known-answer test in \texttt{repclause\_host}) and thereafter
\texttt{Receive\_From} delivers the group's datagrams; this is what mDNS or SSDP would
need. At the far end of the spectrum, \texttt{Open\_MACRAW} (socket~0 only) hands you
whole Ethernet frames and bypasses the offloaded stack entirely --- the escape hatch
for a protocol the chip does not do natively, or your own layer-2 code. Using MACRAW
means giving up the very thing that makes a W5500 pleasant, so it is a deliberate,
last-resort mode, not a default.
...
function Resolve (Server : Inet_Addr_Type; Name : String;
                  ...
                  Local_Port : Port_Type := 0;      --  0: fresh port per query
                  Server_Port : Port_Type := 53) return Boolean;
\end{lstlisting}

\subsection{Four transports for one protocol}\index{DNS!transports}\index{DoT}\index{DoH}

UDP on port~53 is the default, but it is also the most interfered-with
traffic class on the internet: carriers, hotels and national firewalls all
tamper with it, and this project's own cellular bearer swallows it for hours
at a stretch while everything else flows. DNS was designed with escape
hatches for exactly that, and the SDK implements all of them --- one proven
message layer (\texttt{DNS\_Client.Wire} builds the query, validating the RFC
1035 name bounds; \texttt{DNS\_Client.Parse} walks the reply, SPARK-proven
against any malformed input) under four carriages:

\begin{itemize}
\item \textbf{UDP} (\texttt{Resolve}) --- one datagram out, one in.
\item \textbf{TCP} (\texttt{Resolve\_TCP}, RFC~7766) --- the same message
  behind a two-byte length prefix; the standard recourse to a truncated
  (TC-flagged) UDP answer, and the one transport that survived every measured
  cellular blackout.
\item \textbf{DoT} (\texttt{DNS\_TLS.Resolve\_DoT}, RFC~7858, port~853) ---
  that same TCP framing, inside a TLS~1.3 session.
\item \textbf{DoH} (\texttt{DNS\_TLS.Resolve\_DoH}, RFC~8484, port~443) ---
  the query as the body of an HTTP/1.1 \texttt{POST} of
  \texttt{application/dns-message}, in TLS.
\end{itemize}

The encrypted pair (\texttt{libs/tls}, target-only like the rest of the TLS
stack) is the real answer to interference: DoT and DoH are designed to be
indistinguishable from ordinary TLS and HTTPS, so a port-53 interceptor
cannot even see them. As with every TLS user here, the application owns the
session and its trust anchor; \texttt{DNS\_TLS} only supplies the framing.
\texttt{Net\_Resolver} climbs a ladder --- UDP:53, then UDP:443 to an off-port
resolver, then TCP:53 --- each rung dodging a failure the one above it was
measured to have. The \texttt{esp32s3\_dns\_secure} example resolves one name
all four ways against Google Public DNS over Ethernet.

This example is also where a stubborn ``first connection fails'' bug was finally
run to ground, and the chase is instructive because the symptom pointed
everywhere except the culprit. After bring-up, the \emph{first} TCP connection
would establish and send but never receive its reply --- the read timed out ---
while the second connection and every one after worked at once. It looked like a
W5500 silicon wart: register-level tracing confirmed the connect handshake
completed (\texttt{Sn\_SR} = ESTABLISHED), the query went out with the correct
bytes, yet \texttt{Sn\_RX\_RSR} never left zero. On-subnet connections were
immune; UDP was immune; the failure was hard (no recovery in thirty seconds).
The tell came from a throwaway probe that fixed a \emph{different} destination:
whatever state the first connection primed was not per-destination, so it could
not be the remote server or a NAT conntrack entry keyed on it. Bisecting the one
thing that changes between the first facade connection and the second --- the
ephemeral source port --- landed it. The facade handed out ports from the bottom
of the IANA dynamic range, so the board's first-ever connection always left from
port \textbf{49152}, and this network's NAT router mishandled exactly that port:
a connection from 49152 traversing the gateway established and sent but its reply
was dropped, while the identical target from 49153 or 50000 worked, and an
on-subnet peer (no NAT) worked even from 49152. Nothing to do with the W5500 at
all. The fix is one every hardened TCP stack already applies (RFC 6056):
randomise the starting ephemeral port per boot, seeded here from the sub-second
jitter of the first connect, so the board neither camps on the contended bottom
of the range nor reuses the same 4-tuple across resets. The lesson is the older
one --- a symptom at the chip does not mean the cause is at the chip --- and it
is why the resolver ladder and every reconnect loop retry regardless: a fresh
port is drawn each attempt.
...
One contract in that loop is easy to miss: \texttt{Send\_Socket} is a
\emph{partial} send, exactly like POSIX \texttt{send(2)}.\index{partial send}
It writes what fits in the socket's 2\,KB transmit buffer and reports the last
element written (\texttt{Sent} above). The small payloads in these examples
always fit, so they read clean. A bulk sender must loop until \texttt{Sent}
reaches the end of the slice. Forgetting that cost a real bug in an
SSH server built over this socket layer: a key-exchange reply truncated at the
buffer boundary, and the client reported a bad MAC three messages later.
...
\subsection{\texttt{esp32s3\_dns\_secure}: one name, four transports}\index{DNS!transports}

The same name resolved four ways against Google Public DNS --- UDP, TCP, DoT
and DoH (\S\ref{ch:net:dns-dhcp}) --- so the four carriages over the one proven
message layer can be watched side by side. The encrypted pair stands up a real
TLS~1.3 session to \texttt{dns.google} and validates the full served chain up
to a pinned P-384 ECC root before a query byte is sent --- the anchor step
verifying an ECDSA-P384-SHA384 signature with the pure-Ada \texttt{P384}
package; the plain pair is two calls.
Reading it alongside the debugging tale above also shows why the first
connection used to fail --- the facade once handed out ephemeral port 49152,
which this network's NAT mishandled --- and how randomising the ephemeral base
(RFC 6056) fixed it for good, with no per-example workaround.
...
\subsection{\texttt{esp32s3\_w5500\_lowpower}: sleeping the PHY}\index{power saving}

The low-power lever of \S\ref{ch:net:lowpower}. It brings the chip up, then cycles
the PHY down and back up three times, reading the mode back with \texttt{Power} and
watching \texttt{Link} each round. With no cable it still proves the chip took the
command (the read-back flips \texttt{Normal}\,$\leftrightarrow$\,\texttt{Power\_Down});
with a cable in a live switch you watch the link, and the port LED, drop and return.
...
\noindent\textbf{Example:} \texttt{examples/esp32s3\_ftp\_server}, which formats a
...
A board may carry more than one way onto a network --- two W5500s, say a primary and
a backup. The registry of \S\ref{ch:net:netdevice} already allows it:
...
\begin{lstlisting}
Net_Routes.Set_Default (Eth0, Metric => 10);    -- 0.0.0.0/0 out the primary NIC
Net_Routes.Set_Default (Eth1, Metric => 100);   -- ... or the backup, less preferred
Net_Routes.Add_Route ((10,0,0,0), (255,0,0,0), Eth0, Metric => 10);  -- 10/8 is primary
\end{lstlisting}
...
per interface, are decided by metric, so the primary default (10) wins over the
backup default (100). ``The backup link, always last'' is a metric.

The \texttt{Is\_Up} filter is what turns preference into \textbf{failover}. Give both
interfaces a default route, the primary cheaper; normally the primary default wins.
Unplug the primary and its routes drop out of consideration, so the backup default
takes over, automatically, no application code. Plug it back in and it preempts again
...
\subsection{How traffic chooses an interface}
...
UDP follows the same rule, \emph{per datagram}: the \texttt{To}-form of
\texttt{Send\_Socket} resolves its destination through the table and re-homes an
unpinned datagram socket before sending (which is why that overload takes the
socket \texttt{in out}). This was learned on the first Ethernet-plus-cellular
board: \texttt{Create\_Socket} parks every socket on the default interface, and
before datagrams routed, DNS queries left whatever interface happened to be
index~0 --- live or not --- while the standby carried nothing. A bound listener,
or any pinned socket, stays where it was put.
...
\subsection{Beyond one chip: the interface contract}

What lets a second interface slot in so cleanly is that \texttt{Net\_Devices.Device}
is a \emph{socket-level} contract (\texttt{Connect}/\texttt{Listen}/\texttt{Send} by
address and port), not a packet-level one. An offloaded-socket NIC like the W5500
implements it directly, with no on-chip TCP/IP stack to run on the host: its
\texttt{Connect} writes a socket register, and the facade above neither knows nor
cares which interface is underneath. Any other offloaded-socket interface fits the
same way --- one more \texttt{Add\_Interface} call and a route or two, and nothing
above the facade changes. (A raw Ethernet MAC or Wi-Fi part would \emph{not} fit so
cleanly: those are packet-level and would need a software IP stack beneath this
interface.)
