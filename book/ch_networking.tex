...
for one hardware wart it documents: the \emph{first} W5500 TCP connect to an
off-subnet host after bring-up can fail once, the chip failing a SYN sent
before the gateway's ARP entry exists rather than waiting for it --- a retry
(which \texttt{Net\_Resolver} and any reconnect loop already do) succeeds.
