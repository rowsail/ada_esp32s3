...
\texttt{Address} aspect binds the object there. The write is exactly one 32-bit store because the type is exactly
\texttt{Unsigned\_32}: the compiler
...
state that survives sleep, internal SRAM for hot DMA buffers (DMA reaches PSRAM
only cache-line-aligned, through the data cache), flash for constants.}
\label{tab:memmap}
\end{table}
