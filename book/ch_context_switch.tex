...
\textbf{Preemptive} \textemdash{} an OS-band interrupt (the tick or a device
interrupt) fires while a task is running. The vector (\texttt{highint5.S})
saves the interruptee with \texttt{\_xt\_context\_save} into a full
\texttt{XT\_STK} interrupt frame and services the pending sources. At the
\emph{outermost} interrupt exit \textemdash{} and only there \textemdash{} the
epilogue compares \texttt{First\_Thread} with \texttt{Running\_Thread}; if
they differ it tail-jumps \texttt{\_\_gnat\_preempt\_dispatch} to switch away.
This path saves the outgoing task in \emph{interrupt} form.
...
chain corrupts the register windows. So \texttt{Context\_Switch} refuses to run
from interrupt context: while the vector's per-CPU nesting count
(\texttt{\_\_gnat\_int\_nest}) is nonzero it simply returns. No flag is needed
\textemdash{} the outermost epilogue re-derives the decision from the scheduler
queues itself and performs the dispatch via
\texttt{\_\_gnat\_preempt\_dispatch}. (Tracking down a bug here
...
\begin{itemize}
  ...
  \item \textbf{Interrupt} (\texttt{Frame\_Kind = 1}): the full register state is in
        ...
        (the resume tail borrows the otherwise-unused level-5
        \texttt{EPS/EPC} pair purely as scratch for the atomic return).
\end{itemize}
...
\section{The boot window: refusing to switch too early}\index{boot window}\index{port\_xSchedulerRunning}

Between reset and the moment the environment thread is registered, the thread
tables are part-built: one slot may already hold a thread while the other is
still null.  FreeRTOS guards this window with
\texttt{port\_xSchedulerRunning} \textemdash{} its interrupt epilogue will not
dispatch until the scheduler declares itself up.  This port applies the same
rule in both entry paths.  \texttt{Context\_Switch} returns without switching
while a native interrupt is in flight (\texttt{\_\_gnat\_int\_nest} nonzero:
the epilogue owns dispatch there), and likewise while either
\texttt{Running\_Thread\_Table} or \texttt{First\_Thread\_Table} is still
null \textemdash{} there is no context to save into, or none to resume.  The
interrupt epilogue makes the same two null checks before it compares the
tables, and plainly resumes the interruptee when either fails.  The cost is two
loads per switch.  The alternative, observed the hard way, is a jump through a
zeroed context buffer: \texttt{PC = 0} with \texttt{PS.EXCM} set, a double
exception, and a park with the evidence destroyed.
