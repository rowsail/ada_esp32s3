...
\lstexample{A boot counter that survives deep sleep.}
\begin{lstlisting}
...
\lstexample{Hold a relay line high through sleep.}
\begin{lstlisting}
with ESP32S3.GPIO;
...
\lstexample{An RTC pull-up on a wake pin.}
\begin{lstlisting}
with ESP32S3.RTC_IO; use ESP32S3.RTC_IO;
...
\lstexample{Read two pads' baselines.}
\begin{lstlisting}
with Ada.Text_IO;   use Ada.Text_IO;
...
\lstexample{Detect a finger against a captured baseline.}
\begin{lstlisting}
with Ada.Text_IO;   use Ada.Text_IO;
with ESP32S3.Touch; use ESP32S3.Touch;
...
\lstexample{SHA-256 of a message.}
\begin{lstlisting}
...
\lstexample{SHA-1 (e.g. for a legacy checksum).}
\begin{lstlisting}
with ESP32S3.SHA; use ESP32S3.SHA;
...
\lstexample{AES-128 encrypt then decrypt one block.}
\begin{lstlisting}
...
\lstexample{AES-256 (a 32-byte key selects the cipher automatically).}
\begin{lstlisting}
with ESP32S3.AES; use ESP32S3.AES;
