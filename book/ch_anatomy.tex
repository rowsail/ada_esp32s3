...
\item[The 2nd-stage bootloader.] Before any of the above runs, our own minimal
  loader (ZFP Ada throughout --- \texttt{boot\_main} plus \texttt{boot\_psram}
  --- behind an assembly prologue) maps the flash cache/MMU and brings up the
  octal PSRAM, then jumps to the application image. It replaces the chip
  vendor's stock second-stage bootloader, and contains no C.
...
\textbf{Case is insignificant: pick a convention because the compiler will
...
\item Most applications, where you want exceptions, RAII handles, the secondary
...
The same shared driver tree is safely reused across profiles without stale
objects because the HAL keys its object directory by profile.
...
Console output goes through \texttt{ESP32S3.Console}, a small driver for the
built-in USB Serial/JTAG port, so \texttt{Put\_Line} reaches \texttt{/dev/ttyACM0}.
It is a real driver, not a raw \texttt{printf}. It applies USB back-pressure so a
fast burst is not truncated, line-buffers to coalesce small writes, and --- when
no host is attached --- drops output rather than stall the application.

Both front ends ride it. \texttt{ESP32S3.Log} writes to it directly; the runtime's
\texttt{Ada.Text\_IO} is bridged onto it by one installed hook. So a program can
use either and reach the same console, and a single \texttt{Set\_Output} call can
redirect that console to a UART (Section~\ref{driver:console-mux}). Either way,
\texttt{./x run} opens a monitor so you see the output live.
