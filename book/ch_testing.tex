...
routing table, and the language interpreters built on this SDK
...
a buffer to itself and compares. When a UART device refused to answer
...
\section{Layer 7: adversarial stress}\index{stress testing}\index{esp32s3\_stress}

Everything so far checks that the system does the right thing under loads
someone imagined.  The last layer manufactures the loads nobody would:
\texttt{esp32s3\_stress} runs a \emph{tiny-delay storm} (tasks issuing
randomized overdue, zero-length and sub-millisecond \texttt{delay until}s,
mixed with busy-spins so the tick preempts at arbitrary instruction
boundaries) underneath \emph{cross-core wakeup ping-pong} (suspension-object
and protected-entry pairs split across the cores, waking each other at tens
of kilohertz).  Detection needs no console: every task publishes a heartbeat
in RAM, an on-board monitor latches the first stalled slot, and a host
script reads the verdict over USB-JTAG.  The PRNG seed is the boot cycle
counter, published so a failing schedule can be replayed.

Two rules this layer taught.  First, \emph{soak hands-off}: halting the
chip to read heartbeats stops the cores while the system timer keeps
counting, and the monitor will faithfully report the observer's own
disturbance.  Arm the run, walk away, read once at the end.  Second, the
suite earns its keep at a rate no application can match: its first two
minutes reproduced -- and its load then narrowed to an eleven-line fix -- a
cross-core interrupt-nesting defect that months of hours-long application
soaks had never surfaced, because the application never sustained
twenty-thousand cross-core wakeups a second.  Stress is not a bigger
application test; it is a different instrument.
...
sweep and a stress soak, and a change to a parser re-runs its proof. Together they are why the code in
