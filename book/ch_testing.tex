...
board. The TLSF allocator (\texttt{tlsf\_core}, Chapter~\ref{ch:heap}), the LISP
reader and evaluator (Chapter~\ref{ch:lisp}), the ext4 block logic
(Chapter~\ref{ch:storage}), the Modbus framing (Chapter~\ref{ch:modbus}) and the
routing table are all written target-free, so the identical Ada compiles and runs on
...
kernel's recovery. The LISP host test runs a suite of expressions and compares each
result; the Modbus and routing tests drive the framing and the longest-prefix logic
against hand-built vectors. When a host test passes, the logic is right; only the
...
but proves the foundation. A change to the filesystem runs the host suite against
\texttt{e2fsck} in seconds, a change to the curve re-runs a KAT, a change to a driver
re-runs its loopback on the board, and a change to the runtime re-runs the sweep.
Together they are why the code in this book can be trusted to do on silicon what the
prose says it does.
