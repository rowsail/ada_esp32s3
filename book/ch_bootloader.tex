...
bootloader, with its own, written, like almost everything else here, in
Ada.\footnote{The 2nd-stage bootloader and boot code are original Ada, but they
stand on Espressif's groundwork: the memory map and linker scripts, the mask-ROM
function addresses, and the boot/clock/watchdog sequence are transcribed or adapted
from \textbf{ESP-IDF}~v5.4.4 (Apache-2.0, \textcopyright~Espressif Systems), and the
vendored \texttt{xtensa/config} and HAL headers are \textbf{Tensilica Inc.}'s
(MIT-style licence). Full credit to both; see Further Reading
(Chapter~\ref{ch:references}).}
...
it, and DMA reaches it only through the data cache, under a cache-line alignment
contract (\S\ref{ch:perf}). Small DMA scratch buffers are simpler and faster in
internal SRAM, which is what the SD and filesystem drivers use even when the
task stack itself lives in PSRAM.
