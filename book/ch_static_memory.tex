...
example that recurses deeply just exports a bigger value (the SIMD example sets
128\,KiB; the interpreter examples in the companion collection go bigger still).
...
The \texttt{full} profile works differently: it allocates each task's primary
stack \emph{dynamically}, at activation.\index{task!stack in PSRAM}
\index{\_\_gnat\_task\_stack\_alloc} By default the stack comes from the Ada
heap (so with \texttt{HEAP\_PSRAM=1} it lands in PSRAM along with everything
else). For finer control the runtime first probes a weak hook,
\texttt{\_\_gnat\_task\_stack\_alloc(size)}: define that symbol in your
application and every task stack comes from wherever you say. The
engine-monitor example in the companion collection uses exactly this split
\textemdash{} task stacks from a dedicated PSRAM arena, while the heap (and
its secondary stacks) stay in internal DRAM. If the hook, or the heap, cannot
satisfy a request, \texttt{Create\_Task} fails cleanly and activation raises
\texttt{Tasking\_Error} rather than running on a null stack.
...
\texttt{HEAP\_PSRAM=1}, the 8\,MB PSRAM for programs that need real room (the ext4
block cache and the examples-collection interpreter heaps live there).
...
watermark rather than trusting the static sum. The companion
\texttt{esp32s3\_stack\_usage} example shows that watermark live on the board: its
baseline \texttt{used=432} jumps to \texttt{used=6\,512} (of a 16\,KiB env stack)
the moment a deliberately deep call runs, while the static pass, defeated by that
call's recursion, could report only a 384-byte lower bound. Estimate 384,
measured 6\,512: that gap is the entire reason both numbers exist, and why the same
\texttt{Paint\_Env\_Stack}/\texttt{Report} pair belongs in an instrumented TLS fetch
when you need the real figure rather than the floor.
