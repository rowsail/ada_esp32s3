...
\textbf{The coprocessor has to be on --- per task.} The PIE unit is a
\emph{coprocessor}, and on the S3 it is coprocessor~3 (named \texttt{cop\_ai}; the
FPU is coprocessor~0). Coprocessors come out of reset disabled, so the first
\texttt{ee.*} instruction executed with CP3 off takes a coprocessor-disabled
exception --- which on this bare target simply hangs. \texttt{CPENABLE} gates
coprocessor use, and the subtlety that bites is that it is \emph{per task}. The boot
\texttt{start.S} writes \texttt{CPENABLE = 16\#09\#} (bit~0 for the FPU, bit~3 for
PIE, matching the \texttt{XCHAL\_CP\_MASK} the S3 declares) on the startup thread ---
but the GNAT run-time gives every task its own coprocessor context and starts it
with only \texttt{CP0}, the FPU, enabled. Read \texttt{CPENABLE} from inside an
ordinary task and it is \texttt{16\#01\#}: CP3 is off. So enabling PIE in
\texttt{start.S} does \emph{not} carry into the tasks --- a lesson learned the hard
way, with the first board run hanging on the very first vector instruction issued
from the environment task.

The remedy is for each SIMD-using task to turn CP3 on \emph{itself} before its first
vector op, by calling \texttt{ESP32S3.SIMD.Enable}, which ORs bit~3 into
\texttt{CPENABLE} (preserving the FPU bit) and \texttt{rsync}s. It is idempotent and
cheap; do it once at the top of the task. A second consequence is worth stating
plainly: the PIE \texttt{q0}--\texttt{q7} registers are \emph{not} saved and restored
across context switches (unlike the FPU's state), so \texttt{ESP32S3.SIMD} work must
in any case stay within a single task.
...
begin
   Enable;               --  turn CP3 (PIE) on for THIS task -- once, before any op
