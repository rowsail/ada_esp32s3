...
procedure Acquire  (S : in out Session; Host : SPI_Host;
                    Select_CB : CS_Select    := null;       --  app chip select
...
\item[\texttt{Sclk}, \texttt{Mosi}, \texttt{Miso}, \texttt{Cs}] the four lines as
  ...
  defaults to \texttt{No\_Pin} --- with more than one device on the bus, register a
  \texttt{CS\_Select} callback at \texttt{Acquire} instead (below).
\item[\texttt{Select\_CB}, \texttt{Ctx}] an optional application chip-select
  callback and its opaque context (below); leave them out for the hardware
  \texttt{CS0}.
...
\textbf{Application-driven chip select.}\index{chip-select callback}\index{shared SPI bus}
The host has one hardware chip-select (\texttt{CS0}); to put several devices on one
bus, a device can bring its \emph{own} select instead. Pass a \texttt{CS\_Select}
callback --- and an opaque \texttt{Ctx} --- to \texttt{Acquire}: the driver then
suppresses the hardware \texttt{CS0} for that session, and \texttt{Select\_Device}
calls back to assert and deassert the line around the transaction. The select can
be a plain GPIO, several GPIOs into a 3:8 decoder, or an I/O-expander pin --- the
mapping lives entirely in the callback, keyed by \texttt{Ctx}. The callback must be
\emph{library-level and closure-free} (this HAL builds under
\texttt{No\_Implicit\_Dynamic\_Code}, so a nested closure would emit a faulting
trampoline; per-device state travels in \texttt{Ctx}). This is how the W25Q flash
shares the SPI2 bus with the W5500 in Chapter~\ref{ch:storage}.
