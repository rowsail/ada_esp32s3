...
\item[Init-before-use guard.] For the drivers that configure a controller once
  at startup and only later hand out ownership (SPI, I2C, I2S, TWAI, LCD, and
  MCPWM), \texttt{Acquire}/\texttt{Claim} raises \texttt{Not\_Initialized} if the
  port/host/unit was never \texttt{Setup}. Configuration must precede ownership;
  the contract is enforced, not merely documented. (\texttt{UART} goes further:
  it has no separate setup at all, so \texttt{Acquire} claims a bare port and
  \emph{all} configuration runs through the held \texttt{Session}.)
