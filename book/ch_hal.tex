...
\item[Init-before-use guard.] The two shared-bus drivers (SPI, I2C) still
  configure a host once at startup and only later hand out ownership, so
  \texttt{Acquire} raises \texttt{Not\_Initialized} if the host was never
  \texttt{Setup} -- a shared bus's pins and clock are a per-bus concern, not a
  per-acquisition one. The single-peripheral drivers (UART, TWAI, I2S, LCD,
  MCPWM) fold that step away: \texttt{Acquire}/\texttt{Claim} carries the
  configuration itself (or brings the peripheral up lazily on first use), so
  nothing precedes ownership and there is no \texttt{Not\_Initialized}. Either
  way, configuration cannot precede ownership; the contract is enforced.
\item[Ownership gateway.] Across all six \texttt{Session} drivers (SPI, I2C,
  ...
  reconfiguration of pins, baud, inversion, loopback) goes through it. So a
  transfer or a setting change without ownership fails loudly, and a new
  operation cannot be added that skips the check: there is no other door to the
  hardware. This is a project rule for any exclusive driver that can be
  reconfigured under ownership (see the UART section for the worked pattern).
\end{description}
...
\lstexample{A button drives an LED.}
\begin{lstlisting}
with ESP32S3.GPIO; use ESP32S3.GPIO;
procedure Button is
...
\lstexample{Higher resolution and a different span.}
\begin{lstlisting}
with Ada.Text_IO;        use Ada.Text_IO;
with ESP32S3.Temperature; use ESP32S3.Temperature;
...
\lstexample{Write a command to a device.}
\begin{lstlisting}
with Interfaces;
...
\lstexample{Wiring-free self-test via internal loopback.}
\begin{lstlisting}
with Interfaces;
with ESP32S3.SPI; use ESP32S3.SPI;
...
procedure Read  (S : Session; Addr : Slave_Address; Data : out Byte_Array;
                 Success : out Boolean);
procedure Write_Read (S : Session; Addr : Slave_Address;
                      Tx : Byte_Array; Rx : out Byte_Array;
                      Success : out Boolean);
\end{lstlisting}
...
\item[\texttt{Tx}, \texttt{Rx}] (\texttt{Write\_Read}) the bytes to send, then the
  buffer to read back, in one transaction.
...
Payload length is not capped. The controller moves 32 bytes per FIFO load, and the
driver refills or drains that FIFO mid-transaction without letting go of the bus.
So a transfer of any size still reaches the wire as one \texttt{START..STOP}.
Section~\ref{driver:end-opcode} tells that story.

\texttt{Write\_Read} is the third operation, and it is not the same as a
\texttt{Write} followed by a \texttt{Read}. It puts a \emph{repeated} START
between the two halves, with no STOP. The slave therefore sees one command, which
is what a ``name the register, then read it'' device expects.

\lstexample{Write a register on a sensor at address \texttt{16\#68\#}.}
\begin{lstlisting}
...
\lstexample{Register read: name the register, then read it back.}
\begin{lstlisting}
with ESP32S3.I2C; use ESP32S3.I2C;
...
begin
   ...
   Configure_Pins (I2C0, Scl => 9, Sda => 8);
   Acquire (S, I2C0);
   --  One transaction: START, 16#75#, repeated START, read, STOP.
   Write_Read (S, 16#68#, Tx => (1 => 16#75#), Rx => Who, Success => Ok);
end I2C_Read_Reg;
\end{lstlisting}

\noindent Two separate calls, a \texttt{Write} and then a \texttt{Read}, put a
STOP between the register address and the data. Many parts tolerate that, and
some do not: the STOP ends the command. Reach for \texttt{Write\_Read} whenever
the datasheet draws a repeated START.

\section{UART: asynchronous serial}\index{UART driver}

\begin{lstlisting}
--  Acquire claims the port AND configures it in one call:
procedure Acquire (S : in out Session; Port : UART_Port;
                   Baud : Baud_Rate := 115_200;
                   Bits : Data_Bits := 8; Parity : Parity_Mode := None;
                   Stop : Stop_Bits := One;
                   Tx, Rx, Rts, Cts : Optional_Pin := No_Pin;  --  routes pads too
                   Rx_Flow_Threshold : Natural := 100);
procedure Reconfigure (S : Session; ...);  --  re-apply the whole bundle, held port
...
--  Finer reconfiguration -- each also takes the held Session:
...
\item[the \texttt{Set\_*} setters] each take the held \texttt{Session} and one new
  value of the matching \texttt{Acquire} type (e.g. \texttt{Set\_Parity} takes a
  \texttt{Parity\_Mode}); \texttt{Configure\_Pins} additionally accepts per-line
  \texttt{*\_Invert} booleans (default \texttt{False}).
\end{description}

\texttt{Acquire} claims a port \emph{and} shapes it in one call: it hands back a
\texttt{Session}, sets the frame format, and routes the four pins (all optional:
pass only what the link uses, or none for an internal loopback self-test). Every
\texttt{Acquire} applies the full bundle, so a port comes up in exactly the state
you ask for rather than inheriting a previous holder's settings; with all defaults
that is 115200 8-N-1 with no pins. \emph{Every} later change, whether the bundled
\texttt{Reconfigure} or the finer \texttt{Set\_Baud} / \texttt{Configure\_Pins} /
loopback setters, also takes a \texttt{Session} instead of a \texttt{Port}, so you
can change a setting only while you hold the port. Changing a parameter therefore
requires owning the port and can never race another task's transfer. Each frame
setter is an independent read-modify-write that leaves the other attributes and
the pin routing untouched.
...
\textbf{The receive FIFO can lie, and \texttt{Repair\_Rx} makes it stop.}
The S3's UART commits received bytes to its RX FIFO through a small
clock-domain crossing, and two hardware behaviours around that crossing cost
this SDK a full day each to find. First, without the receive-timeout machinery
enabled, the \emph{tail} of a burst sits in the crossing --- invisible to
\texttt{RXFIFO\_CNT}, unreadable --- until more traffic pushes it through. A
polled reader then sees every reply arrive minus its last bytes, forever.
\texttt{Acquire} now enables the timeout at bring-up, so a burst's tail
commits when the line goes idle. Second, an RX FIFO \emph{overflow} can leave
the FIFO's read pointer skewed from its write pointer while the count reads
zero. From then on every read returns the ring's contents \emph{rotated} by
the skew: nothing lost, everything late, replies one exchange behind. Both
faults defeat every check that looks for loss or corruption, because the
stream is merely late.\index{RX FIFO}\index{Repair\_Rx}\index{UART overflow}

\texttt{Repair\_Rx (S)} detects the skew --- pointers that disagree while the
count says empty --- and pulses the FIFO reset until the silicon verifies
aligned. It touches nothing readable, so it is safe whenever the line is known
idle; a request/response protocol calls it just before each request goes out,
and self-heals no matter when the overflow happened. \texttt{esp-idf} resets
the FIFO after every overflow for the same reason.

\cnote{If you have ever watched a peripheral ``answer the previous command''
on an ESP32 and blamed your protocol code, this is the likely culprit. The
arbitrating registers are \texttt{MEM\_RX\_STATUS} (the two pointers) and
\texttt{STATUS.RXFIFO\_CNT}; when they disagree, believe the pointers.}

\lstexample{Transmit a line.}
\begin{lstlisting}
with ESP32S3.UART; use ESP32S3.UART;
procedure UART_Tx is
   S : Session;
begin
   Acquire (S, UART1, Baud => 115_200, Tx => 17, Rx => 16);  --  claim + config
   Write (S, (Character'Pos ('h'), Character'Pos ('i'), 10));
end UART_Tx;
\end{lstlisting}

\lstexample{Echo received bytes (wiring-free, internal loopback).}
\begin{lstlisting}
with ESP32S3.UART; use ESP32S3.UART;
procedure UART_Echo is
   ...
   N   : Natural;
begin
   Acquire (S, UART1);              --  claim UART1 (comes up at 8-N-1 defaults)
...
\begin{enumerate}
\item \texttt{Acquire (S, UART1)} takes exclusive ownership of UART1 and brings it
  up at the default frame format (115200 baud, 8 data bits, no parity, one stop
  bit) with no pins routed, all this wiring-free self-test needs. (\texttt{Acquire}
  applies its configuration bundle on every call; passing no settings here means
  the defaults.) It blocks on UART1's protected guard until the port is free
  (here, immediately), marks \texttt{S} active, and records that it holds UART1.
  Until \texttt{S} is released no other task can acquire UART1, and because every
  configuration call below takes the held \texttt{Session}, no other task can
  change UART1's settings either.
...
Ownership comes first. \texttt{Acquire} (step 1) is the per-transaction gate that
hands you a \texttt{Session} and shapes the port in the same call. Everything
after it (\texttt{Reconfigure}, the finer setters, \texttt{Enable\_Loopback} here,
and the transfers) acts through that \texttt{Session}, so configuring a port is
only possible while you hold it. There is no setup call that precedes ownership:
...
re-running the whole \texttt{Acquire} bundle or touching the pin routing. The
setters take the held \texttt{Session}, so you configure the pins at
\texttt{Acquire} and tweak from there:
\begin{lstlisting}
with ESP32S3.UART; use ESP32S3.UART;
procedure UART_Reconfig is
   S : Session;
begin
   Acquire (S, UART1, Tx => 17, Rx => 16);  --  claim + 115200 8-N-1, pins routed
