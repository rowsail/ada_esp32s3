...
\begin{lstlisting}
procedure Acquire (S : in out Session; Port : UART_Port);  --  claims a bare port
--  All configuration takes the HELD Session, not a Port:
procedure Configure (S : Session; Baud : Baud_Rate := 115_200;
...
\item[the \texttt{Set\_*} setters] each take the held \texttt{Session} and one new
  value of the matching \texttt{Configure} type (e.g. \texttt{Set\_Parity} takes a
  \texttt{Parity\_Mode}); \texttt{Configure\_Pins} additionally accepts per-line
  \texttt{*\_Invert} booleans (default \texttt{False}).
\end{description}

\texttt{Acquire} claims a port and hands back a \texttt{Session}; the first
acquirer brings the port up at a safe default (115200 8-N-1, no pins).
\texttt{Configure} then sets the frame format \emph{and} routes the four pins
(all optional: pass only what the link uses, or none for an internal loopback
self-test) on the port you hold. \emph{Every} configuration call, whether the
bundled \texttt{Configure} or the finer \texttt{Set\_Baud} /
\texttt{Configure\_Pins} / loopback setters, takes a \texttt{Session} instead of
a \texttt{Port}, so you must \texttt{Acquire} the port before you can change any
setting. Changing a parameter therefore requires owning the port and can never
race another task's transfer. Each frame setter is an independent
...
procedure UART_Tx is
   S : Session;
begin
   Acquire (S, UART1);                                   --  claim the port
   Configure (S, Baud => 115_200, Tx => 17, Rx => 16);   --  format + pins in one
...
\begin{enumerate}
\item \texttt{Acquire (S, UART1)} takes exclusive ownership of UART1. The first
  acquirer brings the port up at the default frame format (115200 baud, 8 data
  bits, no parity, one stop bit) with no pins routed, all this wiring-free
  self-test needs. \texttt{Acquire} blocks on UART1's protected guard until the
  port is free (here, immediately), marks \texttt{S} active, and records that it
  holds UART1. Until \texttt{S} is released no other task can acquire UART1, and
  because every configuration call below takes the held \texttt{Session}, no
  other task can change UART1's settings either.
...
hands you a \texttt{Session} and, on first use, brings the port up at safe
defaults. Everything after it (\texttt{Configure}, the finer setters,
\texttt{Enable\_Loopback} here, and the transfers) acts through that
\texttt{Session}, so configuring a port is only possible while you hold it. There
is no setup call that precedes ownership: you cannot touch a UART you do not hold.
...
re-running \texttt{Configure} or touching the pin routing. The setters take the
held \texttt{Session}, so you \texttt{Acquire} the port first: the reconfiguration
...
procedure UART_Reconfig is
   S : Session;
begin
   Acquire (S, UART1);                  --  own the port (comes up 115200 8-N-1)
   Configure (S, Tx => 17, Rx => 16);   --  route the pins
