...
that package is \emph{stripped} --- it can print characters to the console and
read one back, and nothing else (no files, no \texttt{Integer\_IO}, no layout).
Chapter~\ref{ch:runtime-build} explains why: the full package is incompatible
with a configurable run-time.

\texttt{ESP32S3.Text\_IO} fills the gap. It is a near-complete, \emph{pure-Ada}
re-implementation of \texttt{Ada.Text\_IO} that works over two things at once: the
...
(Chapter~\ref{ch:storage}). There is no C anywhere in it --- in particular no
\texttt{Interfaces.C\_Streams}, the C \texttt{FILE*} layer the hosted GNAT
\texttt{Ada.Text\_IO} is built on. Files are reached through the ext4 VFS; numbers
are formatted by hand in Ada.
...
\begin{quote}\small\itshape From C / ESP-IDF: this is the role
\texttt{printf}/\texttt{ESP\_LOGI} play. For terse status logging the lighter
\texttt{ESP32S3.Log} is often enough (Chapter~\ref{ch:hal}); reach for
\texttt{ESP32S3.Text\_IO} when you want files, the numeric generics, or column
layout.\end{quote}

The console is a \texttt{File\_Type} like any other. \texttt{Standard\_Output},
\texttt{Standard\_Error} and \texttt{Standard\_Input} name it, and \texttt{Set\_Output}
redirects the file-less calls:
...
To move the \emph{whole} console onto a UART --- so every byte, including
\texttt{ESP32S3.Log} and the runtime's own \texttt{Ada.Text\_IO} --- use the
device mux instead (Section~\ref{driver:console-mux}):
...
(\texttt{/flash}, \texttt{/sd}), the rest is the path within it. Open or create,
read or write lines, close:
...
the rest of the management surface --- \texttt{Reset} (reposition, or re-open with
a new mode), \texttt{Mode}, \texttt{Name}, \texttt{Flush}, \texttt{Delete} --- all
work as in the Reference Manual.

\textbf{No store, no crash.} Errors are ordinary Ada exceptions, because the ext4
layer's exceptions \emph{are} renames of \texttt{Ada.IO\_Exceptions}. If no volume
is mounted, opening a path on it raises \texttt{Name\_Error}; a read past the end
raises \texttt{End\_Error}; the wrong mode raises \texttt{Mode\_Error}. So a
program that logs to flash degrades to a catchable exception, never a fault:
...
\texttt{"sync=yes"}, which commits after \emph{every} write --- crash-durable, at
the cost of a journal flush per line. An unrecognised form raises
\texttt{Use\_Error}.
...
The full family of numeric and enumeration generics is present, each with
\texttt{Put} \emph{and} \texttt{Get}, the \texttt{To}/\texttt{From} \texttt{String}
variants, and defaults from the type's attributes:
...
\texttt{Decimal\_IO} and \texttt{Enumeration\_IO} are all there. Floating output
formats fixed-point or scientific (when \texttt{Exp > 0}); the fixed-point
generics format through a wide intermediate so a ranged \texttt{delta} type cannot
overflow while being scaled. None of it uses a host \texttt{sprintf}; the digits
are produced in Ada.

\section{Column and line layout}\index{Set\_Col}\index{column layout}

The same line/column/page model the Reference Manual defines is here, which is how
you align columns without counting spaces:
...
\texttt{Integer\_IO}, \texttt{Float\_IO} and the rest --- are not ordinary
generics the compiler reads from your source. GNAT recognises the unit by name and
binds those generics to its \emph{own} predefined implementation units
(\texttt{a-tiinio}, \texttt{a-tiflio}, \dots). You cannot supply your own.
...
I/O, depends on machinery a configurable run-time excludes. The console hook of
Section~\ref{driver:console-mux} does route the stripped package's \texttt{Put\_Line}
through our device mux --- so console-only legacy code already benefits --- but the
files and the generics cannot live under the \texttt{Ada.Text\_IO} name on this
target.
...
the new name). The benefit is a Reference-Manual-shaped, pure-Ada text I/O ---
console and ext4, formatted numbers and column layout --- on a runtime that the
