...
high-bandwidth option), which the boards used here populate with 8\,MB octal PSRAM
(an AP~Memory die in the DevKitC's module, a discrete part on the Meshnology
board). The bring-up does not hard-code the vendor, though: it reads the
...
\texttt{passmask = 0xC3} $\rightarrow$ mode 0, even though their 8\,MB octal PSRAM is
realised differently on each board: in-package inside the module on the DevKitC, a
discrete part on the Meshnology board (and not necessarily the same die at all). That
two different physical realisations land on the same window is the stronger result. The
din window tracks a stable property of the controller and its octal PSRAM, which is
exactly what you want from a calibration: reproducible, not lucky.
...
Task stacks can live here too.\index{PSRAM!task stacks} On the full profile the
runtime allocates each task's primary stack at activation, and the application
can route those allocations into PSRAM: wholesale with \texttt{HEAP\_PSRAM=1}
(stacks follow the heap), or selectively through the
\texttt{\_\_gnat\_task\_stack\_alloc} hook (\S\ref{ch:static-mem}). The
register-window machinery does not care where the stack lives \textemdash{}
window spills and fills go through the same data cache as every other access.
We verified that the hard way: 1.45 million checksum-verified deep recursions
on a PSRAM stack, and an engine-monitor GUI running its whole task set from a
PSRAM arena.

One real caveat remains. PSRAM is unreachable whenever the cache is disabled
(for instance during a flash write), so a task on a PSRAM stack must never run
while flash is being programmed (\S\ref{ch:bootloader}). This runtime never
writes flash at run time. If your application does, keep the flash-writing
task \textemdash{} and every task that may preempt it \textemdash{} on an
internal-SRAM stack.
