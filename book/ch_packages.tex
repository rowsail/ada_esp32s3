...
in Ada, and it shapes this whole codebase. You see it everywhere here -- one
peripheral per package, a tidy \texttt{ESP32S3.*} hierarchy, a private type guarding
every hardware resource, a build that recompiles exactly what changed. The reason is
worth stating up front, because it is unusual. Ada folds into one construct three
things that C keeps separate:
...
separately maintained, free to disagree. Putting them in one package, with a
...
Because clients see only the spec, the body can change completely -- without
breaking, or even recompiling, a single line of client code.
...
W25Q flash, the LCD -- with no copy-paste and no textual inclusion. Each package
compiles on its own to an object plus an \texttt{.ali} (Ada Library Information) file
that records its interface. Clients link against the result; they do not recompile
it.
...
\texttt{new} and \texttt{malloc} was none the wiser -- because the \emph{spec} did
not move.
...
always released'' now belongs to the package -- not to every caller who has to
remember to clean up. Chapter~\ref{ch:encap} covers the mechanics of private types,
...
state,'' ``this unit runs no code at startup'' -- and the compiler \emph{checks}
them. On a Zero-Footprint or light-tasking runtime, that is not a nicety. It is what
lets a package be used where there is no elaboration machinery at all.
