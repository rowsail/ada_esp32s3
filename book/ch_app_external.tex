...
function-specific} I2C command addresses (\texttt{0x24} writes system config,
\texttt{0x23} writes OC outputs, \texttt{0x38} writes IO outputs and \texttt{0x26}
reads IO inputs), so there are no address straps and \texttt{Setup} takes no
...
\texttt{Open\_Drain} drive. The driver keeps a shadow initialised to the datasheet power-on defaults (IO =
inputs, OC = high, push-pull) because the config/OC/IO-output registers cannot
be read back. The chip has no interrupt output,
...
The calendar fields are constrained subtypes (\texttt{Hour\_Number} is
\texttt{0\,..\,23}, \texttt{Year\_Number} \texttt{2000\,..\,2099}, and so on) ---
the right contract for callers, but it means \texttt{Get\_Time} must not decode a
raw register straight into one. A chip that lost power (a dead backup cell) or was
never set can hold out-of-range BCD, and \texttt{Weekday'Val} on the 3-bit day field
can be handed a 7; decoding those directly into the subtypes would raise
\texttt{Constraint\_Error} out of the very call whose job is to \emph{report}
untrustworthy time --- and it would fire precisely when \texttt{Valid} should come
back False. So \texttt{Get\_Time} decodes into unconstrained locals first,
range-checks every field, and builds the \texttt{Time} record only when all are in
range; anything out of range leaves \texttt{T} at its defaults and returns
\texttt{Valid => False} (with \texttt{Result = OK}, since the bus read itself
succeeded). The subtypes still guarantee callers a sane \texttt{Time}; the driver
just stops trusting the wire to supply one.
...
starts a self-looping DMA for gapless, click-free, zero-CPU playback of a
\emph{static} buffer; \texttt{Capture} samples the ADC underneath it without
disturbing playback or the shared master clock. For \emph{live-generated}
audio, \texttt{Play\_Stream} loops a double-buffered ring and
\texttt{Await\_Half} tells the producer which half to refill, paced by the DMA
itself so the synthesis never drifts against the codec clock.
\texttt{Capture\_Stream} is the microphone mirror on its own DMA channel: it
records gaplessly \emph{while} \texttt{Play\_Stream} keeps playing, true full
duplex with no clock gap in either direction (\texttt{Await\_Capture\_Half}
returns each captured half; \texttt{Stop\_Capture} ends it).
\texttt{Duplex} is the one-shot form: play one buffer and capture the ADC in a
single simultaneous transfer. The capture path samples on the true on-wire
clock (the I2S slave-RX clock routing of the previous chapter), so what the
microphone hears lands sample-exact against what the speaker plays: enough to
run an acoustic software-modem loopback, speaker to air to microphone.
...
package ESP32S3.ES8311 is
   ...
   function  Await_Half      (O : Output) return Natural;  --  half to refill (0/1)
   ...
   function  Await_Capture_Half (O : Output) return Natural;  --  half to read
...
\subsection{Proportional fonts and image assets}\index{fonts!anti-aliased}\index{gen\_font.py}\index{asset generators}\label{app:ext:fonts}

The 5$\times$7 \texttt{ST7789.Text} font above is enough for labels; for real
typography the HAL has \texttt{ESP32S3.Fonts}, a panel-independent model for
proportional bitmap fonts at any size, anti-aliased (4-bit, 16-level coverage) or
monochrome (1-bit). A \texttt{Font} is a light descriptor: per-glyph metrics
(advance, width/height, bearings, a byte offset) plus a packed coverage bitmap,
reached by address. The model carries no display dependency --- the generic
\texttt{ESP32S3.Fonts.Render}, instantiated per panel (e.g.\
\texttt{ESP32S3.ST7789.Fonts}), alpha-blends each glyph against the background ---
so the same atlas renders unchanged on any panel.

\begin{lstlisting}
type Font is record
   First, Count   : Natural;             --  code points [First .. First+Count-1]
   Height, Ascent : Natural;             --  line advance; baseline offset
   Bpp            : Natural;             --  1 or 4
   Adv, W, H      : System.Address;      --  per-glyph metric arrays
   XOff, YOff     : System.Address;      --  bearings
   Off            : System.Address;      --  U16 offset into Bits per glyph
   Bits           : System.Address;      --  packed coverage bytes
end record;
\end{lstlisting}

\noindent\textbf{Build-time asset generators.} The atlas is not written by hand:
it is produced from a TrueType font by \texttt{libs/esp32s3\_hal/tools/gen\_font.py},
which rasterises each requested size (Pillow over FreeType) and emits, \emph{per
size}, one pure-Ada package --- the metric arrays and packed 4-bit bitmap as
\texttt{constant} arrays plus an \texttt{ESP32S3.Fonts.Font} over them. The
\texttt{esp32s3\_b612} example wraps the invocation in \texttt{regen.sh} (three
sizes of the B612 typeface). Each size is its own \texttt{-fdata-sections} unit, so
\texttt{with}-ing only the sizes a program uses keeps the rest out of the image ---
a size the app never names costs zero flash. A companion tool,
\texttt{gen\_ada\_logo.py} in \texttt{esp32s3\_gps\_display}, does the same for a
raster image: it rasterises an SVG (Inkscape + Pillow) to a 240$\times$240 RGB565
array and emits a pure-Ada package whose \texttt{Pixels} aggregate is handed
straight to \texttt{Draw\_Bitmap}.

\noindent\textbf{Why Ada data, and why the tools stay Python.} These generators
once emitted C headers that a per-example \texttt{glue.c} \texttt{\#include}d; they
now emit Ada packages directly, so the examples contain no authored C at all.
Keeping the data in Ada is free: a 57\,600-element scalar aggregate compiles in
well under a second. The \emph{generators} themselves stay Python because they are
host build-tools, re-run only when the font or artwork changes, and the heavy
lifting --- TrueType and SVG rasterisation, high-quality resampling --- is
FreeType, Inkscape and Pillow, which have no pure-Ada substitute worth writing for
a tool that emits a committed, rarely-regenerated artefact. The build input is the
generated Ada package, not the tool.

\noindent\textbf{Examples:} \texttt{examples/esp32s3\_b612} (three anti-aliased
sizes), \texttt{examples/esp32s3\_gps\_display} (the boot splash).
...
\section{HC595: SPI Shift-Register Output String}\index{HC595 driver}\index{shift register}
\label{app:ext:hc595}

The 74HC595 is a serial-in / parallel-out shift register: clock eight bits in on
one pin and they appear on eight output pins. Daisy-chained (each chip's overflow
output feeds the next chip's serial input), any number of them form one long
output string, so a few cents of logic buys as many outputs as you like. The
driver treats the chain as exactly that: a \texttt{Controller} sized by a
\texttt{Chips} discriminant, with \texttt{8 * Chips} outputs.

It rides the SPI bus \emph{write-only}: \texttt{MOSI} drives the chip's serial
data (SER) and \texttt{SCLK} the shift clock, so no \texttt{MISO} is needed. Two
ordinary GPIOs complete it: \texttt{RCLK} (the storage-register latch, whose
rising edge copies the shift register to the output pins) and active-low
\texttt{/OE} (output enable). The shared SPI2 bus needs only \texttt{Setup} +
\texttt{Configure\_Pins} first; the driver \texttt{Acquire}s it for each update.

\begin{lstlisting}
package ESP32S3.HC595 is
   subtype Byte is Interfaces.Unsigned_8;
   type Controller (Chips : Positive) is limited private;   --  a string of N chips

   procedure Initialize (C : in out Controller;
                         Host     : ESP32S3.SPI.SPI_Host;
                         RCLK     : ESP32S3.GPIO.Pin_Id;      --  latch (rising edge)
                         OE       : ESP32S3.GPIO.Pin_Id;      --  active-low /OE
                         Clock_Hz : Positive := 10_000_000;
                         Enable   : Boolean  := True);
   function  Output_Count (C : Controller) return Natural;   --  8 * Chips

   --  Stage into an in-RAM shadow, then Update to latch the whole string at once:
   ...
   procedure Update     (C : in out Controller);          --  shift out + pulse RCLK
   ...
   procedure Clear_All (C : in out Controller);  procedure Set_All (C : in out Controller);
   procedure Enable_Outputs (C : in out Controller);
   procedure Disable_Outputs (C : in out Controller);
end ESP32S3.HC595;
\end{lstlisting}

The split between staging and latching is the point. \texttt{Set\_Output} and
\texttt{Set\_Byte} only edit an in-RAM shadow of the string; nothing reaches the
pins until \texttt{Update} shifts the whole shadow out and pulses \texttt{RCLK},
so every output changes together on one clean latch edge with no intermediate
glitch as the bits ripple through. \texttt{Write\_Output} is the one-shot
convenience (set one bit, then \texttt{Update}); \texttt{Clear\_All} /
\texttt{Set\_All} and the \texttt{/OE} \texttt{Enable\_Outputs} /
\texttt{Disable\_Outputs} round it out.

\begin{lstlisting}
declare
   SR : ESP32S3.HC595.Controller (Chips => 3);          --  three chips = 24 outputs
begin
   ESP32S3.SPI.Setup (ESP32S3.SPI.SPI2);                --  bus shared with the flash
   ESP32S3.SPI.Configure_Pins (ESP32S3.SPI.SPI2, Sclk => 1, Mosi => 4,
                               Miso => ESP32S3.GPIO.No_Pin);   --  write-only
   ESP32S3.HC595.Initialize (SR, Host => ESP32S3.SPI.SPI2, RCLK => 5, OE => 6);

   for I in 0 .. ESP32S3.HC595.Output_Count (SR) - 1 loop   --  walk one high output
      ESP32S3.HC595.Clear_All (SR);
      ESP32S3.HC595.Write_Output (SR, I, True);
      delay 0.25;
   end loop;
end;
\end{lstlisting}

\noindent\textbf{Example:} \texttt{examples/esp32s3\_hc595} (a "chase" that walks
one high output across the whole string, for confirming the wiring and chip count
on LEDs or a scope).
...
filesystem (Chapter~\ref{ch:storage}); from the LISP REPL in the companion
examples collection, \texttt{(spi-xfer ...)} reads the same JEDEC id by hand.
