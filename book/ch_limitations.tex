...
\section{Architectural: interrupt-priority tasks and the kernel tick}\index{interrupt priority}

The kernel tick is a level-2 interrupt (the FreeRTOS-model placement; see the
interrupts chapter). A \emph{task} whose priority lies in the
\texttt{Interrupt\_Priority} range therefore runs with the tick's level masked
on its core for as long as it executes. A compute loop at any interrupt
priority starves its core's clock, and a task \emph{declared} at an interrupt
priority can deadlock its own activation (ACATS \texttt{CXD1006}; see the
ACATS chapter).

\textbf{Why it rarely bites.} Protected-object \emph{ceilings} in the
interrupt range are the normal, safe case: they mask the tick only for the
short bounded time a lock is held, and the pending tick fires the moment it is
released. What the model asks is only that \emph{tasks} stay out of the
interrupt-priority range -- ordinary tasks use \texttt{Priority}, and code
that must react at interrupt priority belongs in a protected handler, not a
task. This is the same discipline FreeRTOS asks of its users.
