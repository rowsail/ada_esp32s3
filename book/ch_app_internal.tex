...
Configuration runs entirely through an acquired \texttt{Session}.
\texttt{Acquire} hands out a limited, non-copyable token that owns the port
exclusively (other tasks suspend until it is released) and, in the same call,
fixes baud, frame format and pin routing; all four lines (\texttt{Tx},
\texttt{Rx}, \texttt{Rts}, \texttt{Cts}) are \texttt{Optional\_Pin}, so a link
routes only what it uses. Every \texttt{Acquire} applies the whole bundle, so a
port comes up exactly as asked (115200 8-N-1, no pins by default) rather than
inheriting a previous holder's settings. Every transfer \emph{and} every later
reconfiguration (\texttt{Reconfigure} for the whole bundle, or
\texttt{Set\_Baud}, \texttt{Configure\_Pins}, \texttt{Set\_Inversion}, \ldots for
one attribute) is likewise a method on the held \texttt{Session}, so a reconfigure
can never race another task's transfer. The \texttt{Session} is a
...
package ESP32S3.UART is
   ...
   procedure Reconfigure (S : Session; ...);  --  re-apply the bundle, held port
...
Got  : Natural;
begin
   Acquire (S, Port, Baud => 115_200);  --  claim + 8-N-1, no pins routed
...
the free pool on scope exit, including on an exception). \texttt{Set\_Duty} needs no lock because you exclusively own a claimed channel. The finalization
...
\begin{lstlisting}
Claim (Gen0, MCPWM0, Ch0);                               --  first Claim brings up MCPWM0
...
\texttt{Idle\_Ticks}. Keep higher-priority interrupts short because \texttt{Transmit} busy-polls its
re-fill.
...
This driver targets the embedded/full profile (excluded from the light-tasking
build) because finalization is involved. Transfers are bounded by
...
\texttt{Wait} is interrupt-driven: after a brief spin it suspends the calling
task and the channel's end-of-transfer interrupt wakes it, so a transfer frees
the core rather than busy-waiting. The driver owns \texttt{Device\_L2\_0}
(CPU\_INT 19) for this, so an application must not also attach a handler to it.
...
port. \texttt{Acquire} both claims the port and brings it up, so there is no
separate setup step. The driver needs a tasking runtime (Jorvik or richer) and,
via finalization, targets the embedded/full profile.
...
procedure Reconfigure (S : Session; ...);              --  re-open the held port
procedure Configure_Pins  (S : Session;                --  finer: re-route pins
                           Bclk, Ws, Dout, Din, Mclk : Optional_Pin := No_Pin);
...
declare
   ...
   Ok : Boolean := False;
begin
   Acquire (S, I2S0, Sample_Rate => 16_000, Bits => Bits_16);  --  claim + config
   Enable_Loopback (S, Pad => Data_Pin);    --  out->in on one pad (held port)
...
procedure Configure_Pins   (S : Session; Data : Data_Pins;   --  finer: re-route
...
declare
   ...
   Ok : Boolean;
begin
   Acquire (S, Pclk_Hz => 200 * 1000,             --  200 kHz; claim + configure
            Data => D_Pins, Pclk => Pclk_Pin);
...
procedure Read    (R : Reader; Ch : Channel_Index;
                   Value : out Raw_Value; Valid : out Boolean;
...
declare
   R : Reader;
   High, Low : Raw_Value := 0;
   Ok_High, Ok_Low : Boolean;
begin
   ...
   Read (R, Ch, High, Ok_High);             --  expect near full scale
   ESP32S3.GPIO.Clear (Pin);
   delay until Clock + Milliseconds (2);
   Read (R, Ch, Low, Ok_Low);               --  expect near zero
   --  A reading that did not convert is not evidence of anything: a
   --  timed-out poll leaves the SAR's last latched code in place.
   if not (Ok_High and Ok_Low) then
      raise Program_Error with "ADC did not convert";
   end if;
