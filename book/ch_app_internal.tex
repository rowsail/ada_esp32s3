...
exclusively (other tasks suspend until it is released) and, on first use, brings
the port up at safe defaults (115200 8-N-1, no pins). \texttt{Configure} then
fixes baud, frame format and pin routing on the held port; all four lines
(\texttt{Tx}, \texttt{Rx}, \texttt{Rts}, \texttt{Cts}) are \texttt{Optional\_Pin},
so a link routes only what it uses. Every transfer \emph{and} every later
reconfiguration (\texttt{Set\_Baud}, \texttt{Configure\_Pins},
\texttt{Set\_Inversion}, \ldots) is likewise a method on the held
\texttt{Session}, so a reconfigure can never race another task's transfer. The \texttt{Session} is a
...
Got  : Natural;
begin
   Acquire (S, Port);                 --  claim the port (8-N-1 defaults)
   Configure (S, Baud => 115_200);    --  set the line speed under ownership
